...
export default function BlogDetailPage(){
  ...
  if(error&&!post)return <main className="container section"><div className="error">{error}</div><p><Link href="/blog">← Bloga dön</Link></p></div></main>;
  if(!post)return <main className="container section">Yazı yükleniyor…</div></main>;
  return <main id="theme-section-ticarti-article" data-section-name="main-article"><div className="page-width top-spacing-small">
    <div className="article-template__header text-center"><Link href="/blog" className="link body3">← Blog</Link>{post.category&&<Link className="link body3 text-uppercase" href={'/blog?category='+post.category.slug}>{post.category.title}</Link>}<h1>{post.title}</h1>{post.excerpt&&<p className="body2 mt15">{post.excerpt}</p>}<div className="body3 mt15"><span>{post.publishedAt?new Date(post.publishedAt).toLocaleDateString('tr-TR',{year:'numeric',month:'long',day:'numeric'}):''}</span>{Array.isArray(post.tags)&&post.tags.length>0&&<span>{post.tags.join(' · ')}</span>}</div></div>
    {post.featuredImageUrl&&<img className="article-template__hero image-hover-box mt30" src={post.featuredImageUrl} alt={post.title}/>} 
    <article className="article-template__content rte body2 mt40">{paragraphs(post.body).map((p,i)=><p key={i}>{p.split('\n').map((line,j)=><span key={j}>{j>0&&<br/>}{line}</span>)}</p>)}</article>
    {related.length>0&&<section className="featured-tabs top-spacing-small"><div className="section-header__container"><div><span className="body3 text-uppercase">İLGİLİ ÜRÜNLER</span><h2>Bu yazıdaki ürünler</h2></div><Link href="/products">Tüm ürünler →</Link></div><div className="grid">{related.map((p:any)=><ProductCard key={p.id} p={p} currency={p.displayCurrency||getCurrency()}/>)}</div></section>}
    <section className="article-template__comments top-spacing-small"><div className="section-header__container"><div><span className="body3 text-uppercase">YORUMLAR</span><h2>{post.comments?.length||0} yorum</h2></div></div>{Array.isArray(post.comments)&&post.comments.length>0?<div className="page-grid-1 page-vertical-gap-20 mt20">{post.comments.map((c:any)=><div className="article-comment border__radius" key={c.id}><div><strong>{c.authorName}</strong><span>{new Date(c.createdAt).toLocaleDateString('tr-TR')}</span></div><p>{c.body}</p></div>)}</div>:<p className="muted">Henüz yayınlanmış yorum yok.</p>}
      {canComment?<>{msg&&<div className="notice">{msg}</div>}{error&&<div className="error">{error}</div>}{customer||allowGuest?<form className="article-comment-form mt30" onSubmit={submit}>{!customer&&<div className="form-grid"><label className="field"><span>Adınız</span><input name="authorName" required/></label><label className="field"><span>E-posta</span><input name="authorEmail" type="email"/></label></div>}<label className="field"><span>Yorumunuz</span><textarea name="body" rows={5} minLength={2} required/></label><button className="btn" disabled={busy}>{busy?'Gönderiliyor…':'Yorumu gönder'}</button></form>:<div className="notice">Yorum yapmak için <Link href="/account"><b>hesabınıza giriş yapın</b></Link>.</div>}</>:<div className="notice">Bu yazıda yorumlar kapalı.</div>}
    </section>
  </div></main>;
}
