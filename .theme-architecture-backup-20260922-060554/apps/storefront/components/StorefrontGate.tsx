...
export default function StorefrontGate({children}:{children:React.ReactNode}){
  ...
  return (
    <div className="storefront-theme" style={themeStyle}>
      <LiveTrackerBeacon/>
      <DesignPreviewBridge/>
      <TrackingScripts settings={boot.store?.settings||{}} store={boot.store}/>
      {g.customCss&&<style>{String(g.customCss)}</style>}
      <Header boot={effectiveBoot}/>
      {children}
      <Footer boot={effectiveBoot}/>
      {boot.features?.aiAssistant&&(
        <a
        ...
        >
          <svg viewBox="0 0 24 24" aria-hidden="true">
            <path d="M12 2.5l1.45 4.05L17.5 8l-4.05 1.45L12 13.5l-1.45-4.05L6.5 8l4.05-1.45L12 2.5Zm6.2 8.7.9 2.45 2.4.85-2.4.9-.9 2.4-.85-2.4-2.45-.9 2.45-.85.85-2.45Zm-9.4 4.6 1.1 3.05 3.1 1.1-3.1 1.1L8.8 24l-1.1-2.95-3.05-1.1 3.05-1.1 1.1-3.05Z"/>
          </svg>
        </a>
      )}
      <CookieBanner/>
