...
export default function QuickStoreForm(){
  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    ...
    try{
      ...
      const publicSlug=String(membership?.site?.publicSlug||body.subdomain||'').trim().toLowerCase();
      if(!publicSlug)throw new Error('Mağaza oluşturuldu ancak mağaza adresi belirlenemedi.');
      const url=new URL('/admin/login',`https://${publicSlug}.ticarti.com`);url.searchParams.set('handoff',j.handoffCode);url.searchParams.set('tenant',membership.tenantId);url.searchParams.set('next','/dashboard');window.location.href=url.toString();
