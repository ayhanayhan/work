  ...
  return <div id={`product-${p.id}`} className="product-card text-left image-hover-personal product_card__button_hover" data-product-id={p.id}>
    <div className="product-card__image__container image-hover-personal-element">
      ...
      <div className="product-card__media-buttons product-card__media-buttons--animation product-card__media-buttons_fixed">
        <button type="button" className={`product-card__media-buttons__item ${liked?'active':''}`} onClick={()=>setLiked(toggleSaved('ticarti-wishlist',String(p.id)))} aria-label="Add to wishlist"><Icon kind="heart"/></button>
        <button type="button" className={`product-card__media-buttons__item ${compared?'active':''}`} onClick={()=>setCompared(toggleSaved('ticarti-compare',String(p.id)))} aria-label="Compare"><Icon kind="compare"/></button>
        <button type="button" className="product-card__media-buttons__item" onClick={()=>setQuick(true)} aria-label="Quick View"><Icon kind="eye"/></button>
      </div>
      <div className="badges__container">{soldOut?<span className="badges-item__medium badges-item__sold-out">Sold Out</span>:discount>0&&<span className="badges-item__medium badges-item__sale">-%{discount}</span>}</div>
    </div>
    <div className="product-card__content">
      ...
      <div className="product-card__interface">
        <div className="product-card__form">{v?.id&&<button type="button" disabled={busy||soldOut} className="quick-add__submit btn btn__transition-text btn-card-width" onClick={()=>void quickAdd()}><span className="btn__texts"><span className="btn__top-text is-visible"><Icon kind="cart"/>{soldOut?'Sold Out':busy?'Adding…':'Add to Cart'}</span></span></button>}</div>
        <div className="product-card__media-buttons product-card__media-buttons__inline product-card__media-buttons--mobile"><button type="button" className="product-card__media-buttons__item" onClick={()=>setLiked(toggleSaved('ticarti-wishlist',String(p.id)))} aria-label="Add to wishlist"><Icon kind="heart"/></button><button type="button" className="product-card__media-buttons__item" onClick={()=>setQuick(true)} aria-label="Quick View"><Icon kind="eye"/></button></div>
      </div>
    </div>
    {quick&&<div className="popup-modal quick-view-backdrop" role="dialog" aria-modal="true" aria-label={`${p.title} Quick View`} onClick={()=>setQuick(false)}><div className="popup-modal__content quick-view" onClick={e=>e.stopPropagation()}><button className="quick-view-close" onClick={()=>setQuick(false)} aria-label="Close">×</button><div className="quick-view-media">{first&&<img src={mediaUrl(first.url,'detail','webp')} alt={first.alt||p.title}/>}</div><div className="quick-view-copy">{p.brand?.name&&<small>{p.brand.name}</small>}<h2>{p.title}</h2><div className="price">{money(v?.price,currency)}{v?.comparePrice&&<span className="old">{money(v.comparePrice,currency)}</span>}</div><p>{p.shortDescription||p.description||''}</p><div className="quick-view-buttons"><button className="btn" disabled={busy} onClick={()=>void quickAdd()}>Add to Cart</button><Link className="btn alt" href={'/products/'+p.slug}>View product</Link></div></div></div></div>}
  </div>
}
