...
import { resolveTicartiDesign, resolveTicartiSections, TICARTI_DEFAULT_SKIN } from '../theme-presets/ticarti-source-preset';
...
export class StorefrontService {
  ...
  private themeState(store:any){const root:any=store?.settings&&typeof store.settings==='object'?store.settings:{};return root.themeState&&typeof root.themeState==='object'&&!Array.isArray(root.themeState)?root.themeState:null;}
  private localeCandidates(locale?:string,defaultLocale?:string){const out:string[]=[];for(const raw of [locale,locale?.split('-')[0],defaultLocale,defaultLocale?.split('-')[0]]){const v=String(raw||'').trim();if(v&&!out.includes(v))out.push(v)}return out;}
  private mergeThemeObject(base:any,override:any):any{if(Array.isArray(base)||Array.isArray(override))return override===undefined?base:override;if(base&&typeof base==='object'&&override&&typeof override==='object'){const out:any={...base};for(const [k,v] of Object.entries(override))out[k]=this.mergeThemeObject(out[k],v);return out}return override===undefined?base:override;}
  private localizeThemeNode(value:any,locale?:string,defaultLocale?:string):any{
    if(Array.isArray(value))return value.map(x=>this.localizeThemeNode(x,locale,defaultLocale));
    if(!value||typeof value!=='object')return value;
    const raw:any=value;const out:any={};for(const [k,v] of Object.entries(raw)){if(k==='_i18n')continue;out[k]=this.localizeThemeNode(v,locale,defaultLocale)}
    const map:any=raw._i18n&&typeof raw._i18n==='object'?raw._i18n:{};let localized:any={};for(const key of this.localeCandidates(locale,defaultLocale).reverse()){if(map[key]&&typeof map[key]==='object')localized=this.mergeThemeObject(localized,map[key])}
    if(!Object.keys(localized).length)return out;
    return this.mergeThemeObject(out,this.localizeThemeNode(localized,locale,defaultLocale));
  }
  private designSettings(store:any,locale?:string){
    const state:any=this.themeState(store);
    const legacyDesign:any=(store?.settings as any)?.design||{};
    const skinSlug=String(state?.skinSlug||TICARTI_DEFAULT_SKIN);
    const resolved=state?resolveTicartiDesign(skinSlug,state?.design||{}):legacyDesign;
    const saved:any=this.localizeThemeNode(resolved||{},locale,store?.locale);
    const defaults:any={general:{bodyFont:'Inter, ui-sans-serif, system-ui, sans-serif',headingFont:'Inter, ui-sans-serif, system-ui, sans-serif',baseFontSize:16,h1Size:52,h2Size:34,h3Size:24,primaryColor:'#15171a',secondaryColor:'#727b85',textColor:'#15171a',backgroundColor:'#ffffff',surfaceColor:'#ffffff',borderColor:'#e6e9ec',containerWidth:1180,sectionSpacing:64,borderRadius:8,buttonRadius:6,customCss:''},header:{template:1,sticky:true,topbarEnabled:false,topbarText:'',topbarLink:'',showSearch:false},footer:{template:1,copyright:'© {{year}} {{store_name}}. Tüm hakları saklıdır.',upperHtml:''},products:{categoryTemplate:1,productPageTemplate:1,productCardTemplate:1,columnsDesktop:4,columnsTablet:2,columnsMobile:2}};
  ...
  private async maybeRefreshStoreCurrency(store:any){
    const settings:any=store?.settings&&typeof store.settings==='object'?store.settings:{};const regional:any=settings.regional||{};const exchange:any=regional.exchange||{};if(!exchange.autoUpdate)return;const hours=Math.max(1,Number(exchange.intervalHours||6));const last=exchange.lastUpdatedAt?new Date(exchange.lastUpdatedAt).getTime():0;if(last&&Date.now()-last<hours*3600000)return;const rows:any[]=await this.prisma.storeCurrency.findMany({where:{storeId:store.id,isEnabled:true}});const base=String(store.currency||'TRY').toUpperCase();const targets=rows.map(x=>x.code).filter(x=>x!==base);const rates:Record<string,number>={[base]:1};const source=String(exchange.source||'TCMB').toUpperCase();try{
      if(targets.length&&source==='FRANKFURTER'){const res=await fetch(`https://api.frankfurter.app/latest?from=${encodeURIComponent(base)}&to=${encodeURIComponent(targets.join(','))}`);const json:any=await res.json();if(!res.ok||!json?.rates)return;for(const code of targets)if(Number(json.rates[code])>0)rates[code]=Number(json.rates[code]);}
      ...
      else if(targets.length){const res=await fetch('https://www.tcmb.gov.tr/kurlar/today.xml');const xml=await res.text();if(!res.ok)return;const tryPer:Record<string,number>={TRY:1};const re=/<Currency[^>]*CurrencyCode="([A-Z]{3})"[^>]*>([\s\S]*?)<\/Currency>/g;let m:RegExpExecArray|null;while((m=re.exec(xml))){const unit=Number((m[2].match(/<Unit>([^<]+)<\/Unit>/)||[])[1]||1);const sell=Number((m[2].match(/<ForexSelling>([^<]+)<\/ForexSelling>/)||[])[1]||0);const buy=Number((m[2].match(/<ForexBuying>([^<]+)<\/ForexBuying>/)||[])[1]||0);const value=sell||buy;if(value>0)tryPer[m[1]]=value/Math.max(unit,1);}const baseTry=tryPer[base];if(!baseTry)return;for(const code of targets)if(tryPer[code])rates[code]=baseTry/tryPer[code];}
      for(const row of rows){if(regional.currencyMeta?.[row.code]?.pinned)continue;const rate=row.code===base?1:Number(rates[row.code]||0);if(rate>0)await this.prisma.storeCurrency.update({where:{id:row.id},data:{exchangeRate:rate}});}await this.prisma.tenant.update({where:{id:store.id},data:{settings:{...settings,regional:{...regional,exchange:{...exchange,source,lastUpdatedAt:new Date().toISOString()}}}}});
    }catch{}
  }
  private async currencyInfo(store: any, requested?: string | null) {
    await this.maybeRefreshStoreCurrency(store);
    const [rows,fixedApp] = await Promise.all([
      this.prisma.storeCurrency.findMany({ where: { storeId: store.id, isEnabled: true } }),
      this.prisma.appInstallation.findFirst({where:{tenantId:store.id,enabled:true,status:'ACTIVE',app:{slug:'fixed-currency-prices'}},select:{id:true}}),
    ]);
    ...
    return { code: row?.code || store.currency, rate: Number(row?.exchangeRate || 1), rounding: row?.rounding ?? 2, available: rows.map(x => ({ code: x.code, rate: Number(x.exchangeRate), isDefault: x.isDefault })), fixedPricing:!!fixedApp };
  }
  private money(v: any, rate: number, rounding=2) { const factor = 10 ** rounding; return Math.round(Number(v || 0) * rate * factor) / factor; }
  private productRulePrice(product:any,variant:any,customerGroupId?:string|null){
    const rules:any[]=Array.isArray((product?.metadata as any)?.pricing?.discounts)?(product.metadata as any).pricing.discounts:[];const now=Date.now();let best=Number(variant?.price||0);
    for(const rule of rules){const price=Number(rule?.price||0);if(!(price>=0))continue;const group=String(rule?.customerGroupId||'');if(group&&group!==String(customerGroupId||''))continue;const start=rule?.startsAt?new Date(rule.startsAt).getTime():0;const end=rule?.endsAt?new Date(rule.endsAt).getTime():0;if(start&&start>now)continue;if(end&&end<now)continue;if(price<best)best=price;}
    return best;
  }
  private displayVariantPrice(product:any,variant:any,ci:any,customerGroupId?:string|null){
    const fixed=ci?.fixedPricing?Number((product?.metadata as any)?.pricing?.fixedCurrencyPrices?.[ci.code]?.[String(variant?.sku||'')]||0):0;
    if(fixed>0)return fixed;return this.money(this.productRulePrice(product,variant,customerGroupId),ci.rate,ci.rounding);
  }
  ...
  private presentProduct(p: any, ci: any) {
    return { ...p, displayCurrency: ci.code, variants: (p.variants || []).map((v:any)=>({ ...v, basePrice: Number(v.price), price: this.displayVariantPrice(p,v,ci), comparePrice: v.comparePrice == null ? null : this.money(v.comparePrice, ci.rate, ci.rounding) })) };
  }
  ...
  private async reserveTargets(db:any,targets:any[]){for(const target of targets){if(target.inventory?.trackStock)await db.inventoryItem.update({where:{variantId:target.variantId},data:{reserved:{increment:target.quantity}}});}}
  ...
  async bootstrap(slug: string, currency?: string, locale?: string) {
    ...
    const [categoriesRaw, bannersRaw, menus, featuredRaw, shippingMethods, paymentMethods, legalDocuments, popups, locales, sections, reviews, blogPosts, brands, activeThemeDef] = await Promise.all([
      ...
      this.prisma.product.findMany({ where: { storeId: store.id, status: 'ACTIVE' }, include: { images: { orderBy: { sortOrder: 'asc' }, take: 1 }, brand: true, variants: { where: { isActive: true }, include: { inventory: true }, take: 1 }, categories: { include: { category: true } }, reviews: { where: { status: 'APPROVED' }, select: { rating: true } } }, orderBy: { createdAt: 'desc' }, take: 48 }),
    ...
    const presentedPool = featured.map((p:any)=>this.presentProduct({ ...p, rating: p.reviews?.length ? p.reviews.reduce((a:number,r:any)=>a+Number(r.rating||0),0)/p.reviews.length : null, reviewCount:p.reviews?.length||0 },ci));
    let bestSelling:any[]=[];
    try{
      const sales:any[]=await (this.prisma.orderItem.groupBy as any)({by:['variantId'],where:{variantId:{not:null},order:{storeId:store.id}},_sum:{quantity:true},orderBy:{_sum:{quantity:'desc'}},take:80});
      const variantIds=sales.map((x:any)=>x.variantId).filter(Boolean);const variants=variantIds.length?await this.prisma.variant.findMany({where:{id:{in:variantIds}},select:{id:true,productId:true}}):[];const productByVariant=new Map(variants.map((v:any)=>[v.id,v.productId]));const rank=new Map<string,number>();let pos=0;for(const row of sales){const pid=productByVariant.get(row.variantId as string);if(pid&&!rank.has(pid))rank.set(pid,pos++)}bestSelling=[...presentedPool].filter((p:any)=>rank.has(p.id)).sort((a:any,b:any)=>Number(rank.get(a.id))-Number(rank.get(b.id)));
    }catch{bestSelling=[]}
    const bestRated=[...presentedPool].filter((p:any)=>Number(p.reviewCount||0)>0).sort((a:any,b:any)=>Number(b.rating||0)-Number(a.rating||0)||Number(b.reviewCount||0)-Number(a.reviewCount||0));
    ...
    const state:any=this.themeState(store);const resolvedSections=state?resolveTicartiSections(String(state?.skinSlug||TICARTI_DEFAULT_SKIN),Array.isArray(state?.sections)?state.sections:[]):[];const themeSections=this.localizeThemeNode(resolvedSections,activeLocale,store.locale).filter((x:any)=>x?.enabled!==false);const themeMenus=Array.isArray(state?.menus)&&state.menus.length?this.localizeThemeNode(state.menus,activeLocale,store.locale):menus;return { store: safeStore, categories, banners, menus:themeMenus, featured: presentedPool.slice(0,12), productPools:{newest:presentedPool,bestSelling:bestSelling.length?bestSelling:presentedPool,bestRated:bestRated.length?bestRated:presentedPool}, shippingMethods: shippingMethods.map(x=>({ ...x, price:this.money(x.price,ci.rate,ci.rounding), freeAbove:x.freeAbove==null?null:this.money(x.freeAbove,ci.rate,ci.rounding) })), paymentMethods, legalDocuments, contractPages, popups, sections:themeSections, reviews, blogPosts, brands, features:{aiAssistant:!!aiAssistant}, theme:activeThemeDef||{slug:'nova-commerce',name:'Ticarti Core',version:'1.0.0',config:{}}, design:this.designSettings(store,activeLocale), themeStateVersion:Number(state?.version||0) };
  }
  ...
  async product(slug: string, productSlug: string, currency?: string, locale?: string) {
    ...
    if(!p&&locale&&locale!==store.locale){const trs:any[]=await this.prisma.contentTranslation.findMany({where:{storeId:store.id,entityType:'product',locale}});const hit=trs.find((row:any)=>String((row.fields as any)?.slug||'')===productSlug);if(hit)p=await this.prisma.product.findFirst({where:{id:hit.entityId,storeId:store.id,status:'ACTIVE'},include});}
  ...
  async customerRegister(slug: string, body: any, req?: any) {
    ...
    const customer = existing ? await this.prisma.customer.update({ where: { id: existing.id }, data: { passwordHash: hash, firstName: body.firstName || existing.firstName, lastName: body.lastName || existing.lastName, phone: body.phone || existing.phone } }) : await this.prisma.customer.create({ data: { storeId: store.id, email, passwordHash: hash, firstName: body.firstName, lastName: body.lastName, phone: body.phone } });
  ...
  async customerRefresh(refreshToken:string){ const h=this.tokenHash(refreshToken); const s=await this.prisma.customerSession.findFirst({where:{refreshTokenHash:h,revokedAt:null,expiresAt:{gt:new Date()}},include:{customer:true}}); if(!s) throw new UnauthorizedException('Invalid refresh token'); const next=randomBytes(48).toString('base64url'); await this.prisma.customerSession.update({where:{id:s.id},data:{refreshTokenHash:this.tokenHash(next),lastUsedAt:new Date()}}); const accessToken=await this.jwt.signAsync({sub:s.customer.id,storeId:s.customer.storeId,email:s.customer.email,kind:'customer',sid:s.id},{expiresIn:'15m'}); return {accessToken,refreshToken:next,expiresIn:900}; }
  ...
  private normalizeShippingCountry(value:any){return String(value||'').trim().toUpperCase().slice(0,12);}
  private shippingRuleForCountry(settings:any,country:string){const rules:any[]=Array.isArray(settings?.shippingCountryRules)?settings.shippingCountryRules:[];const code=this.normalizeShippingCountry(country);return rules.find((r:any)=>{const countries=(Array.isArray(r?.countries)?r.countries:[]).map((x:any)=>this.normalizeShippingCountry(x));return countries.includes(code)||countries.includes('*')||countries.includes('ALL');})||null;}
  private async shippingContextForCart(cart:any,ci:any,totals:any,countryOverride?:string){
    const [methods,checkoutConfig]=await Promise.all([
      this.prisma.shippingMethod.findMany({where:{storeId:cart.storeId,isActive:true},orderBy:{sortOrder:'asc'}}),
      this.prisma.checkoutConfig.findUnique({where:{storeId:cart.storeId},select:{settings:true}})
    ]);
    const country=this.normalizeShippingCountry(countryOverride||(cart.shippingAddress as any)?.country||(cart.billingAddress as any)?.country||cart.store?.defaultCountry||'');
    const settings:any=(checkoutConfig as any)?.settings||{};const rule:any=this.shippingRuleForCountry(settings,country);
    const methodIds=new Set((Array.isArray(rule?.methodIds)?rule.methodIds:[]).map(String));const methodCodes=new Set((Array.isArray(rule?.methodCodes)?rule.methodCodes:[]).map((x:any)=>String(x).toLowerCase()));
    let eligible:any[]=methods;if(rule?.disabled===true)eligible=[];else if(methodIds.size||methodCodes.size)eligible=methods.filter((m:any)=>methodIds.has(String(m.id))||methodCodes.has(String(m.code).toLowerCase()));
    const freeByMethod:any=rule?.freeAboveByMethod&&typeof rule.freeAboveByMethod==='object'?rule.freeAboveByMethod:{};
    const availableMethods=eligible.map((m:any)=>{const override=freeByMethod[m.id]??freeByMethod[m.code]??rule?.freeAbove;const freeAboveBase=override==null?m.freeAbove:Number(override);return {...m,price:this.money(m.price,ci.rate,ci.rounding),freeAbove:freeAboveBase==null?null:this.money(freeAboveBase,ci.rate,ci.rounding)};});
    const selectedMethod=availableMethods.find((m:any)=>String(m.id)===String(cart.shippingMethodId||''))||availableMethods[0]||null;
    const netSubtotal=Math.max(0,Number(totals?.subtotal||0)-Number(totals?.discountTotal||0));const threshold=selectedMethod?.freeAbove==null?null:Number(selectedMethod.freeAbove);const promotionFree=!!totals?.freeShipping;const reached=promotionFree||(threshold!=null&&threshold>0&&netSubtotal>=threshold);const remaining=reached||threshold==null?0:Math.max(0,threshold-netSubtotal);const progress=threshold&&threshold>0?Math.max(0,Math.min(100,(netSubtotal/threshold)*100)):(reached?100:0);
    return {country,availableMethods,selectedMethodId:selectedMethod?.id||null,selectedMethod,freeShippingThreshold:threshold,amountUntilFreeShipping:remaining,freeShippingReached:reached,freeShippingByPromotion:promotionFree,progress,ruleApplied:!!rule};
  }
  async getCart(token:string){const cart=await this.prisma.cart.findUnique({where:{token},include:{items:{include:{variant:{include:{product:{include:{images:{orderBy:{sortOrder:'asc'},take:1},categories:true}},inventory:true}}}},store:true,customer:true}});if(!cart)throw new NotFoundException('Cart not found');await this.prisma.cart.update({where:{id:cart.id},data:{lastActivityAt:new Date(),...(cart.abandonedAt?{recoveredAt:new Date(),abandonedAt:null}:{})}});const totals=await this.calculateCart(cart);const ci=await this.currencyInfo(cart.store,cart.currency);const displayItems=cart.items.map((i:any)=>({...i,variant:{...i.variant,basePrice:Number(i.variant.price),price:this.displayVariantPrice(i.variant.product,i.variant,ci,cart.customer?.groupId)}}));
    const relationIds=new Set<string>();const bundleOffers:any[]=[];const rawGroups:any[]=[];
    for(const item of cart.items as any[]){const cfg:any=(item.variant?.product?.metadata as any)?.together||{};const relation=(Array.isArray(cfg.productIds)?cfg.productIds:[]).map(String).filter((id:string)=>id!==String(item.variant?.productId));for(const id of relation)relationIds.add(id);if(relation.length)rawGroups.push({anchorProductId:String(item.variant.productId),anchorTitle:String(item.variant?.product?.title||''),productIds:relation});if(cfg.bundleEnabled&&relation.length)bundleOffers.push({anchorProductId:item.variant.productId,anchorTitle:item.variant?.product?.title||'',productIds:relation,bundlePrice:cfg.bundlePrice==null?null:this.money(cfg.bundlePrice,ci.rate,ci.rounding)});}
    const existingProductIds=new Set((cart.items as any[]).map((x:any)=>String(x.variant?.productId||'')));const ids=[...relationIds].filter(id=>!existingProductIds.has(id));const related=ids.length?await this.prisma.product.findMany({where:{id:{in:ids},storeId:cart.storeId,status:'ACTIVE'},include:{images:{orderBy:{sortOrder:'asc'},take:1},variants:{where:{isActive:true},include:{inventory:true},take:1}}}):[];const togetherSuggestions=related.map((p:any)=>this.presentProduct(p,ci));const byId=new Map(togetherSuggestions.map((p:any)=>[String(p.id),p]));const togetherGroups=rawGroups.map((g:any)=>({...g,suggestions:g.productIds.map((id:string)=>byId.get(id)).filter(Boolean)})).filter((g:any)=>g.suggestions.length);
    const shippingContext=await this.shippingContextForCart(cart,ci,totals);
    return{...cart,items:displayItems,totals,promotionGifts:totals.gifts,togetherSuggestions,togetherGroups,bundleOffers,shippingContext};}
  ...
  async checkoutData(token:string){
    ...
    return{
      ...
      shippingMethods:Array.isArray(cart.shippingContext?.availableMethods)?cart.shippingContext.availableMethods:shippingMethods.map((x:any)=>({...x,price:this.money(x.price,ci.rate,ci.rounding),freeAbove:x.freeAbove==null?null:this.money(x.freeAbove,ci.rate,ci.rounding)})),
      shippingContext:cart.shippingContext,
  ...
  async selectShipping(token:string,shippingMethodId:string){const cart:any=await this.getCart(token);if(cart.status!=='active')throw new BadRequestException('Cart is not active');const allowed=new Set((cart.shippingContext?.availableMethods||[]).map((x:any)=>String(x.id)));if(!allowed.has(String(shippingMethodId||'')))throw new BadRequestException('Select a shipping method available for this country');await this.prisma.cart.update({where:{id:cart.id},data:{shippingMethodId:String(shippingMethodId),lastActivityAt:new Date()}});return this.getCart(token);}
  ...
  async prepareCheckout(token:string,body:any,customerId?:string){const cart:any=await this.getCart(token);if(!cart.items.filter((x:any)=>!x.isGift).length)throw new BadRequestException('Cart is empty');if(customerId){const customer=await this.prisma.customer.findFirst({where:{id:customerId,storeId:cart.storeId,isActive:true},select:{id:true}});if(!customer)throw new UnauthorizedException('Customer does not belong to this store');}const ci=await this.currencyInfo(cart.store,cart.currency);const checkoutShipping=await this.shippingContextForCart({...cart,shippingAddress:body.shippingAddress||cart.shippingAddress,billingAddress:body.billingAddress||body.shippingAddress||cart.billingAddress},ci,cart.totals,String(body?.shippingAddress?.country||''));const allowed=new Set((checkoutShipping.availableMethods||[]).map((x:any)=>String(x.id)));if(!allowed.has(String(body.shippingMethodId||'')))throw new BadRequestException('Select a shipping method available for the delivery country');const shipping=await this.prisma.shippingMethod.findFirst({where:{id:String(body.shippingMethodId),storeId:cart.storeId,isActive:true}});if(!shipping)throw new BadRequestException('Select a valid shipping method');const payment=await this.prisma.paymentMethod.findFirst({where:{id:body.paymentMethodId,storeId:cart.storeId,isActive:true}});if(!payment)throw new BadRequestException('Select a valid payment method');await this.prisma.cart.update({where:{id:cart.id},data:{customerId:customerId||cart.customerId,customerEmail:body.email,shippingMethodId:shipping.id,paymentMethodId:payment.id,shippingAddress:body.shippingAddress,billingAddress:body.billingAddress||body.shippingAddress,note:body.note}});return{...(await this.getCart(token)),paymentMethod:payment,shippingMethod:shipping};}
  async checkout(token:string,body:any,customerId?:string,req?:any){if(body.consents && (body.consents.terms===false || body.consents.privacy===false)) throw new BadRequestException('Required legal acknowledgements are missing');const idempotencyKey=String(body.idempotencyKey||'').trim()||null;const cartIdentity=await this.prisma.cart.findUnique({where:{token},select:{id:true,storeId:true}});if(!cartIdentity)throw new NotFoundException('Cart not found');if(body.checkoutSessionId){const checkoutSession=await this.prisma.checkoutSession.findFirst({where:{id:String(body.checkoutSessionId),storeId:cartIdentity.storeId,cartId:cartIdentity.id,status:'OPEN'}});if(!checkoutSession)throw new BadRequestException('Invalid checkout session');if(checkoutSession.expiresAt.getTime()<=Date.now())throw new BadRequestException('Checkout session expired');}if(idempotencyKey){const existing=await this.prisma.order.findUnique({where:{storeId_idempotencyKey:{storeId:cartIdentity.storeId,idempotencyKey}},include:{items:true,customer:true}});if(existing){if(body.checkoutSessionId)await this.prisma.checkoutSession.updateMany({where:{id:String(body.checkoutSessionId),cartId:cartIdentity.id,storeId:cartIdentity.storeId,status:'OPEN'},data:{status:'COMPLETED',completedAt:new Date(),orderId:existing.id}});return{order:existing,payment:{type:'existing',code:existing.paymentMethod,requiresExternalPayment:false}};}}const prepared:any=await this.prepareCheckout(token,body,customerId);const cart:any=prepared;const payment=prepared.paymentMethod;const activeSub=await this.prisma.subscription.findFirst({where:{tenantId:cart.storeId,status:{in:['TRIAL','ACTIVE']}},include:{plan:true},orderBy:{createdAt:'desc'}});if(activeSub?.plan.maxOrdersPerMonth){const now=new Date();const monthStart=new Date(now.getFullYear(),now.getMonth(),1);const count=await this.prisma.order.count({where:{storeId:activeSub.tenantId,createdAt:{gte:monthStart}}});if(count>=activeSub.plan.maxOrdersPerMonth)throw new BadRequestException('Monthly order limit reached for current plan');}
  ...
  async requestReturn(customerId:string,orderId:string,body:any){const order=await this.prisma.order.findFirst({where:{id:orderId,customerId},include:{items:true}});if(!order)throw new NotFoundException('Order not found');if(!['FULFILLED','PROCESSING'].includes(order.status))throw new BadRequestException('Order is not eligible for return');const requested=Array.isArray(body.items)?body.items:[];return this.prisma.returnRequest.create({data:{storeId:order.storeId,orderId,customerId,reason:body.reason,note:body.note,items:{create:requested.map((x:any)=>{const oi=order.items.find(i=>i.id===x.orderItemId);if(!oi)throw new BadRequestException('Invalid return item');return{orderItemId:oi.id,variantId:oi.variantId,quantity:Math.min(Number(x.quantity||1),oi.quantity)};})}},include:{items:true}});}
}
