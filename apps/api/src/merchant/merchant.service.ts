...
export class MerchantService {
  ...
  private themeSkinRowsFromDb(config:any,planId?:string){
    ...
    return ticartiSkinCatalog().map((skin:any)=>{
      ...
      const m:any=meta[skin.slug]||{};const planPricing:any[]=Array.isArray(m.planPricing)?m.planPricing:[];const configured=planPricing.length>0;const planRow:any=planId?planPricing.find((x:any)=>String(x.planId)===String(planId)&&x.enabled!==false):null;const available=!configured||!!planRow;const included=!configured||!!planRow?.included||Number(planRow?.price||0)<=0;const price=included?0:Number(planRow?.price||0);
      ...
      return {slug:String(skin.slug),name:String(m.name||row.name||skin.name||skin.slug),category:String(row.category||skin.category||skin.group||''),previewImageUrl:String(m.previewImageUrl||row.previewImageUrl||skin.previewImageUrl||''),verifiedFromDemo:!!skin.verifiedFromDemo,translations:m.translations||{},planPricing,isActive:m.isActive!==false,isFeatured:!!m.isFeatured,available,included,price,currency:String(planRow?.currency||'TRY'),design:{...design,general:{...(design.general||{}),themeId:'ticarti',themePackageId:'ticarti',skinSlug:String(skin.slug),skin:String(skin.slug)}},homePreset:homePreset.map((x:any,i:number)=>({sourceId:String(x.sourceId||x.settings?.__sourceId||`${skin.slug}-${i}`),sectionType:String(x.sectionType||'rich_text'),sortOrder:Number(x.sortOrder??i),enabled:x.enabled!==false,settings:{...(x.settings||{}),__sourceId:String(x.sourceId||x.settings?.__sourceId||`${skin.slug}-${i}`)}})),sourceManagedBy:overrides[skin.slug]?'PLATFORM_OVERRIDE':'SOURCE_FILES'};
  ...
  async activateTheme(tenantId:string,storeId:string,themeId:string,options:any={}){
    ...
    const sub=await this.prisma.subscription.findFirst({where:{tenantId,status:{in:['ACTIVE','TRIAL']}},orderBy:{createdAt:'desc'},select:{planId:true}});const cfg:any=theme.config||{};const skins=this.themeSkinRowsFromDb(cfg,sub?.planId);const requested=String(options?.skinSlug||options?.skin||cfg?.defaultSkin||skins.find((x:any)=>x.available)?.slug||skins[0]?.slug||'').trim();const source=skins.find((x:any)=>x.slug===requested);if(!source)throw new BadRequestException('Tema bulunamadı.');if(source.available===false)throw new BadRequestException('Bu tema mevcut paketinizde kullanılamaz.');if(!source.included&&Number(source.price)>0)throw new BadRequestException(`Bu tema ${source.price} ${source.currency} ek ücretlidir. Satın alma akışı tamamlanmadan etkinleştirilemez.`);
