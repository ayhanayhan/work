...
function schemaValue(values:any,id:string){if(values?.[id]!==undefined)return values[id];const runtime=BLOCK_FIELD_TO_RUNTIME[id];return runtime&&values?.[runtime]!==undefined?values[runtime]:''}
function patchSchemaValue(values:any,id:string,value:any){const out={...(values||{}),[id]:value};const runtime=BLOCK_FIELD_TO_RUNTIME[id];if(runtime)out[runtime]=value;return out}
...
export default function DesignViewRouter({page,data,sid,req,reload}:Props){
  ...
  const editorSourceMode=String(params.get('sourceMode')||'edit');
  ...
  const[design,setDesign]=useState<any>(()=>editorMode&&editorSourceMode==='demo'?{}:clone(data.design||{}));
  const[sections,setSections]=useState<any[]>(()=>editorMode&&editorSourceMode==='demo'?[]:clone(arr(data.sections)));
  const[menus,setMenus]=useState<any[]>(()=>editorMode&&editorSourceMode==='demo'?[]:clone(arr(data.menus)));
  ...
  const demoSnapshotReadyRef=useRef(false);
  const designRef=useRef<any>(design);
  const sectionsRef=useRef<any[]>(sections);
  const menusRef=useRef<any[]>(menus);
  ...
  function previewOrigin(){try{return previewUrl?new URL(previewUrl,window.location.origin).origin:window.location.origin}catch{return window.location.origin}}
  function sendDraft(message:any){const frame=previewRef.current?.contentWindow;if(!frame)return;frame.postMessage({source:'commerce-design-editor',action:'draft',...message},previewOrigin())}
  function pushFullDraft(){const frame=previewRef.current?.contentWindow;if(!frame)return;const origin=previewOrigin();frame.postMessage({source:'commerce-design-editor',action:'draft',kind:'design',design:designRef.current},origin);frame.postMessage({source:'commerce-design-editor',action:'draft',kind:'sections',sections:sectionsRef.current},origin);frame.postMessage({source:'commerce-design-editor',action:'draft',kind:'menus',menus:menusRef.current},origin);frame.postMessage({source:'commerce-design-editor',action:'select',target:selectedRef.current},origin)}

  useEffect(()=>{if(editorMode)return;setDesign(clone(data.design||{}));setSections(clone(arr(data.sections)));setMenus(clone(arr(data.menus)));setDirty(false)},[editorMode,data.design,data.sections,data.menus]);

  useEffect(()=>{try{const saved=sessionStorage.getItem('ticarti-design-save-success');if(saved){sessionStorage.removeItem('ticarti-design-save-success');setMsg(saved)}}catch{}},[]);
  ...
  useEffect(()=>{
    ...
    if(editorSkinParam)url.searchParams.set('ticarti_skin',editorSkinParam);
    url.searchParams.set('ticarti_source',editorSourceMode);
    url.searchParams.set('ticarti_debug','1');
    setPreviewReady(false);
    setPreviewUrl(url.toString());
  },[pageKey,editorSkinParam,editorSourceMode]);

  useEffect(()=>{
    const frame=previewRef.current?.contentWindow;
    if(!frame)return;
    frame.postMessage({source:'commerce-design-editor',action:'select',target:selected},previewOrigin());
  },[selected,previewUrl]);
  ...
  useEffect(()=>{
    const onPreviewMessage=(event:MessageEvent)=>{
      ...
      if(payload.ready){pushFullDraft();setPreviewReady(true);return;}
  ...
  },[pageKey,editorSourceMode,editorThemeId,editorSkinParam]);

  const selectedSection=useMemo(()=>{if(!selected.startsWith('section:'))return null;const key=selected.slice(8);return sections.find((x:any)=>[x?.id,x?.sourceId,x?.settings?.__sourceId].some(v=>String(v||'')===key))||null},[selected,sections]);
  const themeName=String(((themes.find((t:any)=>String(t.id)===editorThemeId)||themes.find((t:any)=>t.current)||themes[0])?.config?.skins||[]).find((x:any)=>String(x.slug)===String(editorSkinParam||design?.general?.skinSlug||design?.general?.skin||''))?.name||(themes.find((t:any)=>String(t.id)===editorThemeId)||themes.find((t:any)=>t.current)||themes[0])?.name||'Tema');
  ...
  const demoEditorLoading=editorMode&&editorSourceMode==='demo'&&!previewReady;
  ...
  function sourceFor(t:any,skinSlug:string){const skins=Array.isArray(t?.config?.skins)?t.config.skins:[];const skin=skins.find((x:any)=>String(x.slug)===String(skinSlug))||skins[0]||null;if(!skin)return {skin:null,design:{general:{skinSlug:String(skinSlug||''),skin:String(skinSlug||'')}},sections:[]};const resolvedSlug=String(skin.slug||skinSlug);const sourceDesign=clone(skin.design||{});sourceDesign.general={...(sourceDesign?.general||{}),skinSlug:resolvedSlug,skin:resolvedSlug,skinName:String(skin.name||resolvedSlug),themeBundleSkinSlug:resolvedSlug};const preset=clone(Array.isArray(skin.homePreset)?skin.homePreset:[]);return {skin,design:sourceDesign,sections:preset.map((x:any,i:number)=>{const sourceId=String(x.sourceId||x.settings?.__sourceId||`${resolvedSlug}-${i}`);return {id:`draft:${sourceId}:${i}`,sourceId,pageKey:'home',sectionType:String(x.sectionType||'rich_text'),sortOrder:i,enabled:x.enabled!==false,settings:{...(x.settings||{}),__sourceId:sourceId}}})}}
  ...
  function openThemeEditor(theme:any,skinSlug:string,sourceMode:'edit'|'demo'='edit'){const url=new URL(adminBrowserPath('/design-content'),window.location.origin);url.searchParams.set('editor','1');url.searchParams.set('themeId',String(theme.id));url.searchParams.set('skinSlug',String(skinSlug));url.searchParams.set('sourceMode',sourceMode);window.location.assign(url.toString())}

  function updateDesign(group:string,key:string,value:any){setDesign((current:any)=>{const next={...current,[group]:{...(current?.[group]||{}),[key]:value}};designRef.current=next;sendDraft({kind:'design',design:next});return next});setDirty(true);setMsg('')}
  function updateSection(id:string,patch:any){setSections(current=>{const next=current.map(row=>String(row.id)===id?{...row,...patch,settings:patch.settings?{...(row.settings||{}),...patch.settings}:row.settings}:row);sectionsRef.current=next;const section=next.find(row=>String(row.id)===id);if(section)sendDraft({kind:'section',section});return next});setDirty(true);setMsg('')}

  function updateMenus(next:any[]){menusRef.current=next;setMenus(next);sendDraft({kind:'menus',menus:next});setDirty(true);setMsg('')}
  ...
  useEffect(()=>{if(!editorMode||!themes.length)return;const target=themes.find((t:any)=>String(t.id)===editorThemeId)||themes.find((t:any)=>t.current)||themes[0];if(!target)return;const skinSlug=String(editorSkinParam||chosenSkin(target));const key=`${target.id}:${skinSlug}:${editorSourceMode}`;if(draftInitRef.current===key)return;draftInitRef.current=key;selectedRef.current='';setSelected('');setPanelMode('root');setPageKey('home');setMsg('');if(editorSourceMode==='demo'){const source=sourceFor(target,skinSlug);demoSnapshotReadyRef.current=true;setPreviewReady(false);designRef.current=source.design;sectionsRef.current=source.sections;menusRef.current=[];setDesign(source.design);setSections(source.sections);setMenus([]);setDirty(true);setMsg('Tema veritabanı kaynağı yüklendi. Mağazaya kurmak için Kaydet’e basın.');return;}const savedDesign=clone(data.design||{});const savedSections=clone(arr(data.sections));const savedMenus=clone(arr(data.menus));designRef.current=savedDesign;sectionsRef.current=savedSections;menusRef.current=savedMenus;setDesign(savedDesign);setSections(savedSections);setMenus(savedMenus);setDirty(false);queueMicrotask(()=>{sendDraft({kind:'design',design:savedDesign});sendDraft({kind:'sections',sections:savedSections});sendDraft({kind:'menus',menus:savedMenus})})},[editorMode,themes,editorThemeId,editorSkinParam,editorSourceMode,data.design,data.sections,data.menus]);
  ...
  async function saveAll(){
    ...
    try{
      const targetTheme=themes.find((t:any)=>String(t.id)===editorThemeId)||currentTheme;
      if(!targetTheme?.id)throw new Error('Kaydedilecek tema bulunamadı.');
      const targetSkin=String(editorSkinParam||design?.general?.skinSlug||design?.general?.skin||currentSkinSlug);
      const cleanGeneral={...(design?.general||{})};delete cleanGeneral.themeName;
      ...
      try{setThemes(await req('/admin/themes?storeId='+encodeURIComponent(sid)))}catch{}
      setMsg('Tasarım başarıyla kaydedildi ve mağazada yayınlandı.');
      try{setHistory(await req('/admin/design/history?storeId='+encodeURIComponent(sid)))}catch{}
      if(editorSourceMode==='demo'){
        try{sessionStorage.setItem('ticarti-design-save-success','Tema başarıyla mağazaya kaydedildi ve yayınlandı.')}catch{}
        const url=new URL(window.location.href);url.searchParams.set('sourceMode','edit');url.searchParams.set('themeId',String(targetTheme.id));url.searchParams.set('skinSlug',targetSkin);window.location.replace(url.toString());return;
      }
  ...
  if(page==='overview'&&!editorMode){
    ...
    return <div className="design-v16-library-page">
      ...
      {sourceTheme&&<section className="design-v16-current"><h2>Seçili tema</h2><div className="design-v16-current-card"><ThemeThumb slug={savedSkinSlug} image={sourceTheme.previewImageUrl}/><div className="design-v16-current-copy"><b>{activeSkin?.name||sourceTheme.name}</b><span>{sourceTheme.name} · {savedSkinSlug}</span><p>{sourceTheme.description||'Mağazanızda kullanılan aktif tema.'}</p></div><button type="button" onClick={()=>openThemeEditor(sourceTheme,savedSkinSlug,'edit')}>Edit theme</button></div></section>}
      <section className="design-v16-discover"><div className="design-v16-section-title"><div><h2>Temaları keşfet</h2><p>Bir tema seçtiğinizde canlı tasarım alanında açılır; kaydedene kadar mağazanız etkilenmez.</p></div></div><div className="design-v16-theme-grid">{sourceTheme&&skins.map((skin:any,i:number)=><button type="button" className={'design-v16-theme-card '+(String(skin.slug)===savedSkinSlug?'active':'')} key={skin.slug} onClick={()=>openThemeEditor(sourceTheme,String(skin.slug),'demo')}><ThemeThumb slug={String(skin.slug)} image={skin.previewImageUrl||sourceTheme.previewImageUrl} index={i}/><span className="design-v16-theme-name">{skin.name}</span><small>{skin.category||sourceTheme.category||'Tema'}</small></button>)}</div></section>
    </div>
  }
  ...
  const sectionsPanel=<>
    <SidebarSubHeader title="Anasayfa Bölümleri" subtitle={demoEditorLoading?'Tema modülleri hazırlanıyor…':`${sections.length} bölüm`} onBack={panelBack} action={demoEditorLoading?undefined:<button type="button" onClick={()=>setPanelMode('library')}><Icon icon="iconoir:plus"/> Ekle</button>}/>
    <div className="design-v14-tree-scroll design-v15-section-scroll">
      {demoEditorLoading?<div className="design-v15-empty"><Icon icon="iconoir:refresh-double"/><b>Tema hazırlanıyor</b><span>Seçili temanın gerçek demo modülleri yükleniyor.</span></div>:sections.length?<div className="design-v14-section-list">{sections.map((section:any,index:number)=><SectionTreeItem key={section.id} section={section} index={index} active={selected==='section:'+section.id} onSelect={()=>openSettings('section:'+section.id)} onToggle={()=>updateSection(String(section.id),{enabled:section.enabled===false})} onDuplicate={()=>void duplicateSection(section)} onDelete={()=>void deleteSection(String(section.id))} onDragStart={()=>setDragId(String(section.id))} onDrop={()=>dropOn(String(section.id))}/>)}</div>:<div className="design-v15-empty"><Icon icon="iconoir:component"/><b>Henüz bölüm yok</b><span>Gerçek mağaza içeriğini kullanacak ilk bölümü ekleyin.</span></div>}
  ...
  const themesPanel=<>
    ...
    <div className="design-v14-tree-scroll">
      ...
      <div className="design-theme-list">{themes.map((t:any)=>{const skins=Array.isArray(t.config?.skins)?t.config.skins:[];const skinSlug=chosenSkin(t);const skin=chosenSkinConfig(t);const previewDesign=skin?.design||t.config?.design;const previewPreset=Array.isArray(skin?.homePreset)?skin.homePreset:t.config?.homePreset;return <div key={t.id} className={'design-theme-card '+(t.current?'active':'')}>
        ...
        <div className="design-theme-actions"><button type="button" className="design-v15-light-btn" onClick={()=>{const frame=previewRef.current?.contentWindow;if(!frame)return;frame.postMessage({source:'commerce-design-editor',action:'draft',kind:'theme',theme:t,skinSlug},previewOrigin());if(previewDesign){const pd={...previewDesign,general:{...(previewDesign.general||{}),skinSlug,skin:skinSlug,skinName:skin?.name||skinSlug}};frame.postMessage({source:'commerce-design-editor',action:'draft',kind:'design',design:pd},previewOrigin())}if(loadThemeModules&&Array.isArray(previewPreset))frame.postMessage({source:'commerce-design-editor',action:'draft',kind:'sections',sections:previewPreset.map((x:any,i:number)=>({id:x.sourceId||('theme-preview-'+i),sourceId:x.sourceId,pageKey:'home',sectionType:x.sectionType,sortOrder:i,enabled:x.enabled!==false,settings:{...(x.settings||{}),...(x.sourceId?{__sourceId:x.sourceId}:{})}}))},previewOrigin())}}>Önizle</button>
          {(t.current||t.owned||t.included||Number(t.price)===0)?<button type="button" className="design-v15-primary-btn" onClick={()=>openThemeEditor(t,skinSlug,t.current&&String(skinSlug)===String(savedSkinSlug)?'edit':'demo')}>{t.current&&String(skinSlug)===String(savedSkinSlug)?'Düzenle':'Canlı düzenle'}</button>:<button type="button" className="design-v15-primary-btn" onClick={async()=>{try{const out=await req('/admin/billing/iyzico/theme-checkout',{method:'POST',body:JSON.stringify({themeId:t.id})});if(out?.included||out?.owned){setThemes(await req('/admin/themes?storeId='+encodeURIComponent(sid)));return}if(out?.paymentUrl)window.location.href=out.paymentUrl}catch(e:any){setThemeErr(e.message)}}}>Satın al</button>}
        </div>
      </div>})}</div>
    </div>
  </>;
  ...
  return <div className="design-editor-v14 design-editor-v15">
    <div className="design-v14-topbar">
      <div className="design-v14-topbar-left">
        <button type="button" className="design-v14-back" aria-label="Panele dön" onClick={()=>guardNavigate('/design-content')}><Icon icon="iconoir:arrow-left"/></button>
    ...
    <div className={'design-v14-workspace design-v15-workspace '+(sidebarCollapsed?'sidebar-collapsed':'')}>
      <aside className="design-v15-sidebar">
        ...
        <div className="design-v15-sidebar-shell">
          ...
          <div className="design-v16-editor-actions">
            <button type="button" className="design-v16-reset" onClick={()=>{if(!window.confirm('Seçili temanın düzenlenmemiş kurulum ayarlarına dönülsün mü? Kaydetmediğiniz değişiklikler silinecek.'))return;const target=themes.find((t:any)=>String(t.id)===editorThemeId)||currentTheme;if(!target)return;const skinSlug=String(editorSkinParam||design?.general?.skinSlug||currentSkinSlug);const source=sourceFor(target,skinSlug);designRef.current=source.design;sectionsRef.current=source.sections;setDesign(source.design);setSections(source.sections);setSelected('');selectedRef.current='';setPanelMode('root');setDirty(true);setErr('');setMsg('Kurulum ayarları yüklendi. Mağazaya uygulamak için Kaydet’e basın.');queueMicrotask(()=>{sendDraft({kind:'design',design:source.design});sendDraft({kind:'sections',sections:source.sections});sendDraft({kind:'select',target:''})})}}><Icon icon="iconoir:restart"/> Kurulum ayarlarına dön</button>
      ...
      <main className="design-v14-canvas design-v15-canvas">
        <div className={'design-v14-device '+viewport}>
          ...
          <div className="design-v15-preview-stage">{previewUrl?<iframe key={previewUrl} ref={previewRef} title="Canlı Önizleme" sandbox="allow-scripts allow-same-origin allow-forms" src={previewUrl} onLoad={()=>{if(editorSourceMode!=='demo')pushFullDraft()}} style={{opacity:previewReady?1:0,pointerEvents:previewReady?'auto':'none'}}/>:null}{(!previewUrl||!previewReady)&&<div className="design-v15-preview-loading" style={{position:'absolute',inset:0,zIndex:4,display:'grid',placeItems:'center',background:'#fff'}}>Tema önizlemesi hazırlanıyor…</div>}</div>
        </div>
      </main>
    </div>
  </div>;
...
function SettingsPanel({selected,section,design,menus,locales,updateMenus,updateDesign,updateSection,onDelete}:{selected:string;section:any;design:any;menus:any[];locales:any[];updateMenus:(menus:any[])=>void;updateDesign:(group:string,key:string,value:any)=>void;updateSection:(patch:any)=>void;onDelete:()=>void}){
  ...
  if(selected==='header')return <Panel title="Header" subtitle="Logo, duyuru, araçlar ve davranış" icon="iconoir:panel-top">
    <TemplateMini count={4} value={num(design?.header?.template,1)} label="Header" onChange={v=>{updateDesign('header','template',v);updateDesign('header','layout',v===2?'header-layout-tiles':v===3?'header-layout-closed':v===4?'header-layout-vertical':'header-layout-classic')}}/>
  ...
  if(section)return <Panel title={sectionTitle(section)} subtitle={blockTitle(section.sectionType)} icon={blockIcon(section.sectionType)}><SectionSettings section={section} update={updateSection} locales={locales}/><button type="button" className="btn btn-outline-danger w-100 mt-3" onClick={onDelete}><Icon icon="iconoir:trash" className="me-1"/>Bölümü Sil</button></Panel>;
...
function ThemeSettings({design,update}:{design:any;update:(group:string,key:string,value:any)=>void}){const g=design?.general||{};const source=design?.themeSettings?.general||{};return <>
...
function ProductPanel({selected,design,update}:{selected:string;design:any;update:(group:string,key:string,value:any)=>void}){const p=design?.products||{};if(selected==='category')return <><TemplateMini count={8} value={num(p.categoryTemplate,1)} label="Kategori" onChange={v=>update('products','categoryTemplate',v)}/><div className="design-v14-help mt-3">Wokiee koleksiyon düzenleri: sol filtre, yatay filtre, mobil çekmece, filtresiz, sağ filtre ve liste görünümleri.</div><label className="design-v14-field mt-3"><span>Filtre yerleşimi</span><select value={String(p.filterLayout||'auto')} onChange={e=>update('products','filterLayout',e.target.value==='auto'?'':e.target.value)}><option value="auto">Şablona göre otomatik</option><option value="sidebar">Sol sidebar</option><option value="sidebar-right">Sağ sidebar</option><option value="horizontal">Üstte yatay</option><option value="drawer">Çekmece</option><option value="none">Filtre yok</option></select></label><Switch label="Filtrelemeyi etkinleştir" checked={p.enableFiltering!==false} onChange={v=>update('products','enableFiltering',v)}/><Switch label="Grid / liste görünüm değiştirici" checked={p.dynamicGrid!==false} onChange={v=>update('products','dynamicGrid',v)}/><GridFields p={p} update={update}/></>;if(selected==='product-card')return <><TemplateMini count={4} value={num(p.productCardTemplate,1)} label="Kart" onChange={v=>update('products','productCardTemplate',v)}/><div className="design-v14-help mt-3">Kart 1 klasik Wokiee, Kart 2 açıklamalı, Kart 3/4 galeri kontrollü varyasyonlardır.</div><Switch label="Kart galerisini göster" checked={p.cardGallery!==false} onChange={v=>update('products','cardGallery',v)}/><Switch label="Favori butonunu göster" checked={p.showWishlist!==false} onChange={v=>update('products','showWishlist',v)}/><Switch label="Karşılaştırma butonunu göster" checked={p.showCompare!==false} onChange={v=>update('products','showCompare',v)}/><GridFields p={p} update={update}/></>;return <><TemplateMini count={8} value={num(p.productPageTemplate,1)} label="Ürün" onChange={v=>update('products','productPageTemplate',v)}/><div className="design-v14-help mt-3">8 ürün şablonu Wokiee galeri davranışlarına bağlandı: dikey/yatay thumbnail, slider, iki kolon galeri, stacked, geniş iki kolon, sağ thumbnail ve compact.</div><Switch label="Favori özelliği" checked={p.showWishlist!==false} onChange={v=>update('products','showWishlist',v)}/><Switch label="Karşılaştırma özelliği" checked={p.showCompare!==false} onChange={v=>update('products','showCompare',v)}/></>}
...
function blockSchemaForItem(schema:any,item:any){
  const blocks=(Array.isArray(schema?.blocks)?schema.blocks:[]).filter((b:any)=>b?.type!=='@app');
  if(blocks.length<=1)return blocks[0]||null;
  const explicit=String(item?.__blockType||item?.blockType||item?.type||'').replaceAll('_','-').toLowerCase();
  if(explicit){const hit=blocks.find((b:any)=>String(b?.type||'').replaceAll('_','-').toLowerCase()===explicit);if(hit)return hit}
  let best=blocks[0],bestScore=-1;
  for(const block of blocks){let score=0;for(const def of block?.settings||[]){const id=String(def?.id||'');if(!id)continue;if(item?.[id]!==undefined)score+=4;const runtime=BLOCK_FIELD_TO_RUNTIME[id];if(runtime&&item?.[runtime]!==undefined)score+=3;if(id==='text'&&item?.text!==undefined)score+=6;if(id==='image'&&item?.imageUrl!==undefined)score+=5;if(id==='collection'&&(item?.collection!==undefined||item?.title!==undefined))score+=2}if(score>bestScore){bestScore=score;best=block}}
  return best;
}
...
function LegacySectionSettings({section,update,locales}:{section:any;update:(patch:any)=>void;locales:any[]}){const s=section.settings||{};return <>
  ...
  {!['spacer','divider'].includes(section.sectionType)&&<><Field label="Görsel URL" value={s.imageUrl||''} onChange={v=>update({imageUrl:v})} placeholder="https://..."/><Field label="Link" value={s.linkUrl||''} onChange={v=>update({linkUrl:v})} placeholder="/products"/></>}
  <LocalizedSectionContent settings={s} locales={locales} onChange={next=>update(next)}/>
</>}

const LOCALIZED_TEXT_KEYS=['title','subtitle','content','buttonText','heading_text','main_heading','main_subheading','desc','main_desc','button_name','text','label','caption','role'];
function LocalizedSectionContent({settings,locales,onChange}:{settings:any;locales:any[];onChange:(patch:any)=>void}){
  const list=(Array.isArray(locales)&&locales.length?locales:[{locale:'tr-TR',label:'Türkçe'}]).map((x:any)=>({locale:String(x.locale||x.code||'tr-TR'),label:String(x.label||x.locale||x.code||'Dil')}));
  const[active,setActive]=useState(list[0]?.locale||'tr-TR');const i18n:any=settings?._i18n&&typeof settings._i18n==='object'?settings._i18n:{};const current:any=i18n[active]||{};
  const keys=Array.from(new Set(LOCALIZED_TEXT_KEYS.filter(k=>settings?.[k]!==undefined||current?.[k]!==undefined)));
  const items=Array.isArray(settings?.items)?settings.items:[];
  function patchRoot(key:string,value:any){onChange({_i18n:{...i18n,[active]:{...current,[key]:value}}})}
  function patchItem(index:number,key:string,value:any){const next=items.map((item:any,i:number)=>{if(i!==index)return item;const map:any=item?._i18n&&typeof item._i18n==='object'?item._i18n:{};return {...item,_i18n:{...map,[active]:{...(map[active]||{}),[key]:value}}}});onChange({items:next})}
  return <><SectionHead text="Dil içerikleri"/><div className="design-v23-lang-tabs">{list.map(x=><button type="button" key={x.locale} className={active===x.locale?'active':''} onClick={()=>setActive(x.locale)}>{x.label}</button>)}</div><div className="design-v14-help">Başlık, açıklama ve buton metinleri seçili mağaza diline göre saklanır. Görsel, link ve teknik ayarlar ortak kalır.</div>{keys.map(key=><Field key={key} label={`${key} · ${active}`} value={current?.[key]??''} onChange={v=>patchRoot(key,v)} placeholder={String(settings?.[key]??'')}/>)}{items.map((item:any,index:number)=>{const map:any=item?._i18n||{};const loc:any=map[active]||{};const itemKeys=Array.from(new Set(LOCALIZED_TEXT_KEYS.filter(k=>item?.[k]!==undefined||loc?.[k]!==undefined)));if(!itemKeys.length)return null;return <details key={item.id||index} className="design-v22-schema-group"><summary>Blok #{index+1} · {active}</summary>{itemKeys.map(key=><Field key={key} label={key} value={loc?.[key]??''} onChange={v=>patchItem(index,key,v)} placeholder={String(item?.[key]??'')}/>)}</details>})}</>;
}

function SchemaSettingsGroups({groups,values,onChange}:{groups:any[];values:any;onChange:(key:string,value:any)=>void}){return <>{(groups||[]).map((group:any,gi:number)=><details key={gi} className="design-v22-schema-group"><summary>{sourceLabel(group.name||group.theme_name,`Ayar grubu ${gi+1}`)}</summary><SchemaSettingsList settings={group.settings||[]} values={values} onChange={onChange}/></details>)}</>}
function SchemaSettingsList({settings,values,onChange}:{settings:any[];values:any;onChange:(key:string,value:any)=>void}){return <div className="design-v22-schema-fields">{(settings||[]).map((def:any,index:number)=><SchemaField key={`${def.id||def.type}-${index}`} def={def} value={def.id?schemaValue(values,def.id):''} onChange={value=>def.id&&onChange(def.id,value)}/>)}</div>}
function SectionHead({text}:{text:string}){return <div className="design-v14-section-head">{text}</div>}

function SchemaField({def,value,onChange}:{def:any;value:any;onChange:(value:any)=>void}){const type=String(def?.type||'text');const label=sourceLabel(def?.label||def?.content||def?.id||'Ayar');if(type==='header')return <SectionHead text={label}/>;if(type==='paragraph')return <div className="design-v14-help">{label}</div>;if(!def?.id)return null;if(type==='checkbox')return <Switch label={label} checked={!!value} onChange={onChange}/>;if(type==='color'||type==='color_background')return <ColorField label={label} value={value||def.default||'#000000'} onChange={onChange}/>;if(type==='range'||type==='number')return <NumberField label={label} value={value===''?(def.default??0):value} min={Number(def.min??0)} max={Number(def.max??10000)} suffix={def.unit||undefined} onChange={onChange}/>;if(type==='select'||type==='radio')return <label className="design-v14-field"><span>{label}</span><select value={value??def.default??''} onChange={e=>onChange(e.target.value)}>{(def.options||[]).map((opt:any,i:number)=><option value={String(opt.value??'')} key={i}>{sourceLabel(opt.label,opt.value)}</option>)}</select></label>;if(type==='textarea'||type==='richtext'||type==='html'||type==='liquid')return <Area label={label} value={value??def.default??''} onChange={onChange} rows={type==='html'||type==='liquid'?7:4}/>;if(type==='image_picker')return <div className="design-v22-image-field"><Field label={label} value={typeof value==='string'?value:(value?.url||'')} onChange={onChange} placeholder="https://..."/>{typeof value==='string'&&value&&<img src={value.startsWith('//')?'https:'+value:value} alt=""/>}</div>;if(type==='url'||type==='video_url'||type==='video')return <Field label={label} value={value??def.default??''} onChange={onChange} placeholder="https://..."/>;return <Field label={label} value={typeof value==='object'?JSON.stringify(value):value??def.default??''} onChange={onChange}/>}
