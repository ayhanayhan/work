...
export class MerchantAuthService {
  ...
  async exchangeHandoff(code: string, req: any) {
    ...
    await this.prisma.merchantAuthHandoff.update({ where: { id: row.id }, data: { usedAt: new Date() } });
  ...
  async resetPassword(dto: MerchantResetPasswordDto, req: any) {
    ...
    await this.prisma.$transaction(async tx => {
      ...
      await tx.merchantPasswordReset.update({ where: { id: row.id }, data: { usedAt: new Date() } });
