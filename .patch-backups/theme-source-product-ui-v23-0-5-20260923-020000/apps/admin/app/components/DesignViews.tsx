  ...
  useEffect(()=>{
    const onPreviewMessage=(event:MessageEvent)=>{
      ...
      if(payload.snapshot&&editorSourceMode==='demo'){
        ...
        if(snapshot.design&&Array.isArray(snapshot.sections)){
          const key=`${editorThemeId}:${editorSkinParam}:demo`;
          demoSnapshotReadyRef.current=true;
          designRef.current=clone(snapshot.design);
          sectionsRef.current=clone(snapshot.sections);
          menusRef.current=clone(Array.isArray(snapshot.menus)?snapshot.menus:[]);
          setDesign(clone(snapshot.design));
          setSections(clone(snapshot.sections));
          if(Array.isArray(snapshot.menus))setMenus(clone(snapshot.menus));
          setDirty(false);
          setMsg('Tema demosu canlı kaynaktan yüklendi.');
        }
        return;
      }
