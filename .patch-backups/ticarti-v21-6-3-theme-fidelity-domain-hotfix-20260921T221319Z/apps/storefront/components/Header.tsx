...
export default function Header({boot}:{boot:any}){
  ...
  return <>
    {header.topbarEnabled&&<div className="announcement-bar section-background"><div className="page-width text-center body3">{header.topbarLink?<Link className="link link-hover-span" href={header.topbarLink}>{header.topbarText||'Duyuru'}</Link>:<span>{header.topbarText||'Duyuru'}</span>}</div></div>}
    <div className="section-header"><header className={`header__main ${header.sticky===false?'':'sticky-header'} header-template-${header.template||1}`}>
      <div className="page-width header__container display-flex align-items-center">
        <button className="menu-drawer__button header__button hide-lg" type="button" aria-label="Menü" onClick={()=>setMobileOpen(true)}><Icon name="icon-hamburger"/></button>
        <div className="header__heading"><Link className="header__heading-link clear-underline" href="/" aria-label={store.name||'Anasayfa'}>{store.logoUrl?<img className="header__heading-logo" src={store.logoUrl} alt={store.name||'Logo'}/>:<span className="h5">{store.name||'MAĞAZA'}</span>}</Link></div>
        <nav className="list-menu list-menu--classic hide-sp hide-st" aria-label="Ana menü"><ul className="unstyle-ul list-menu--inline">{menuItems.map((x,i)=><li className="list-menu--hover" key={x.id||i}><Link className="body2 list-menu__item strong" href={x.href}>{x.label}{x.children.length>0&&<span aria-hidden="true">⌄</span>}</Link>{x.children.length>0&&<div className="submenu--megamenu submenu-container submenu--to-left"><div className="page-width page-grid-4">{x.children.map((child,j)=><div key={child.id||j} className="mt-first-0"><Link className="h6 clear-underline animation-underline" href={child.href}>{child.label}</Link>{child.children.length>0&&<ul className="unstyle-ul mt10">{child.children.map((leaf,k)=><li key={leaf.id||k}><Link className="body2 link link-hover-span" href={leaf.href}>{leaf.label}</Link></li>)}</ul>}</div>)}</div></div>}</li>)}</ul></nav>
        <div className="header__icons display-flex align-items-center">
          ...
          <Link className="header__button body2 hide-sp" href="/account" aria-label="Hesabım"><Icon name="icon-account"/></Link>
          <Link className="header__button body2 hide-sp" href="/account?tab=wishlist" aria-label="Favoriler"><Icon name="icon-heart"/></Link>
          <button className="header__button body2" type="button" aria-label={`Sepet, ${count} ürün`} onClick={()=>void openCart()}><Icon name="icon-cart"/><span className="menu-drawer__bubble-count">{count}</span></button>
        </div>
      </div>
    </header></div>
