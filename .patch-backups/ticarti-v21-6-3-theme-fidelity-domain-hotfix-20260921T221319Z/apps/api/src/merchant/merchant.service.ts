...
export class MerchantService {
  ...
  async activateTheme(tenantId:string,storeId:string,themeId:string,options:any={}){
    ...
    const preset:any[]=Array.isArray(selectedSkin?.homePreset)?selectedSkin.homePreset:(Array.isArray(cfg.homePreset)?cfg.homePreset:[]);const themeDesign:any=cfg.design||{};const skinDesign:any=selectedSkin?.design||{};const currentDesign:any=await this.designSettings(tenantId,storeId);const nextDesign={general:{...currentDesign.general,...(themeDesign.general||{}),...(skinDesign.general||{}),...(selectedSkin?{skinSlug:selectedSkin.slug,skinName:selectedSkin.name,skinGroup:selectedSkin.group}: {})},header:{...currentDesign.header,...(themeDesign.header||{}),...(skinDesign.header||{})},footer:{...currentDesign.footer,...(themeDesign.footer||{}),...(skinDesign.footer||{})},products:{...currentDesign.products,...(themeDesign.products||{}),...(skinDesign.products||{})}};
