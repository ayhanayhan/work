...
export default function DesignViewRouter({page,data,sid,req,reload}:Props){
  ...
  useEffect(()=>{
    const onPreviewMessage=(event:MessageEvent)=>{
      ...
      if(payload.snapshot&&editorSourceMode==='demo'){
        ...
        demoSnapshotReadyRef.current=true;
      ...
      if(payload.ready){if(editorSourceMode!=='demo')pushFullDraft();return;}
  ...
  async function saveAll(){
    ...
    try{
      ...
      const nextDesign={...design,general:{...cleanGeneral,skinSlug:targetSkin,skin:targetSkin,themeBundleSkinSlug:targetSkin,themeBundleSectionsSkinSlug:targetSkin,themeBundleVersion:'v23.0.7'}};
      const sectionPayload=sections.map((x:any,i:number)=>({sourceId:x.sourceId||x.settings?.__sourceId,sectionType:x.sectionType,sortOrder:i,enabled:x.enabled!==false,settings:x.settings||{}}));
      let out:any;
      try{out=await req('/admin/design/publish',{method:'POST',body:JSON.stringify({storeId:sid,themeId:targetTheme.id,skinSlug:targetSkin,design:nextDesign,pageKey:'home',sections:sectionPayload})})}
      catch(primary:any){
        const text=String(primary?.message||'');
        if(!/Cannot POST|404|Not Found/i.test(text))throw primary;
        await req('/admin/themes/'+encodeURIComponent(String(targetTheme.id))+'/activate',{method:'POST',body:JSON.stringify({storeId:sid,skinSlug:targetSkin,skin:targetSkin})});
        const savedDesign=await req('/admin/design/settings',{method:'PATCH',body:JSON.stringify({storeId:sid,design:nextDesign})});
        const savedSections=await req('/admin/design/sections/snapshot',{method:'PATCH',body:JSON.stringify({storeId:sid,pageKey:'home',items:sectionPayload})});
        out={ok:true,design:savedDesign,sections:savedSections,compatibilityFallback:true};
      }
      ...
      const saved=clone(out?.design||nextDesign);const savedSections=clone(Array.isArray(out?.sections)?out.sections:sections);
      designRef.current=saved;sectionsRef.current=savedSections;setDesign(saved);setSections(savedSections);setDirty(false);setMsg('Tasarım kaydedildi ve mağazada yayınlandı.');
      setThemes(await req('/admin/themes?storeId='+encodeURIComponent(sid)));setHistory(await req('/admin/design/history?storeId='+encodeURIComponent(sid)));
      if(editorSourceMode==='demo'){try{sessionStorage.setItem('ticarti-design-save-success','Tema mağazaya kaydedildi ve yayınlandı.')}catch{}const url=new URL(window.location.href);url.searchParams.set('sourceMode','edit');url.searchParams.set('themeId',String(targetTheme.id));url.searchParams.set('skinSlug',targetSkin);window.location.replace(url.toString());return;}
    ...
    <div className={'design-v14-workspace design-v15-workspace '+(sidebarCollapsed?'sidebar-collapsed':'')}>
      <aside className="design-v15-sidebar">
        ...
        <div className="design-v15-sidebar-shell">
          ...
          <div className="design-v16-editor-actions">
            <button type="button" className="design-v16-reset" onClick={()=>{if(!window.confirm('Seçili temanın düzenlenmemiş kurulum ayarlarına dönülsün mü? Kaydetmediğiniz değişiklikler silinecek.'))return;const target=themes.find((t:any)=>String(t.id)===editorThemeId)||currentTheme;if(!target)return;const skinSlug=String(editorSkinParam||design?.general?.skinSlug||currentSkinSlug);const url=new URL(window.location.href);url.searchParams.set('editor','1');url.searchParams.set('themeId',String(target.id));url.searchParams.set('skinSlug',skinSlug);url.searchParams.set('sourceMode','demo');url.searchParams.set('sourceReset',String(Date.now()));window.location.replace(url.toString())}}><Icon icon="iconoir:restart"/> Kurulum ayarlarına dön</button>
      ...
      <main className="design-v14-canvas design-v15-canvas">
        <div className={'design-v14-device '+viewport}>
          ...
          {previewUrl?<iframe ref={previewRef} title="Canlı Önizleme" sandbox="allow-scripts allow-same-origin allow-forms" src={previewUrl} onLoad={()=>pushFullDraft()}/>:<div className="design-v15-preview-loading">Canlı mağaza hazırlanıyor…</div>}
        </div>
      </main>
    </div>
  </div>;
