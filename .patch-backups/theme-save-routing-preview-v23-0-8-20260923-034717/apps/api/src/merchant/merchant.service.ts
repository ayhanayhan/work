...
export class MerchantService {
  ...
  private ticartiCatalogWithOverrides(config:any){
    ...
    return ticartiSkinCatalog().map((skin:any)=>{const row:any=overrides[skin.slug]||{};const source=this.ticartiSourceOverride(config,skin.slug);return {...skin,...(row.name?{name:String(row.name)}:{}),...(row.category?{category:String(row.category)}:{}),design:source.design,homePreset:source.homePreset,sourceManagedBy:'SUPERADMIN'};});
  }
