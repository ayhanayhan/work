...
export default function DesignViewRouter({page,data,sid,req,reload}:Props){
  ...
  function guardNavigate(url:string){if(dirty&&!window.confirm('Kaydetmediğiniz değişiklikler var. Çıkarsanız yaptıklarınız kaybolacak. Devam edilsin mi?'))return;window.location.href=url}
  function openThemeEditor(theme:any,skinSlug:string){const url=new URL('/design-content',window.location.origin);url.searchParams.set('editor','1');url.searchParams.set('themeId',String(theme.id));url.searchParams.set('skinSlug',String(skinSlug));window.location.assign(url.toString())}
