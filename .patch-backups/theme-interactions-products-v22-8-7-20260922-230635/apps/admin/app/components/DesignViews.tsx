...
const BLOCK_FIELD_TO_RUNTIME:Record<string,string>={heading_text:'title',main_heading:'title',collection_name:'title',desc:'content',main_desc:'content',button_name:'buttonText',button_link_1:'linkUrl',url:'linkUrl',custom_link:'linkUrl',image:'imageUrl',image_mobile:'imageUrlMobile',author_description:'role'};
...
export default function DesignViewRouter({page,data,sid,req,reload}:Props){
  ...
  useEffect(()=>{if(!editorMode||!themes.length)return;const target=themes.find((t:any)=>String(t.id)===editorThemeId)||themes.find((t:any)=>t.current)||themes[0];if(!target)return;const skinSlug=String(editorSkinParam||chosenSkin(target));const key=`${target.id}:${skinSlug}:${editorSourceMode}`;if(draftInitRef.current===key)return;draftInitRef.current=key;selectedRef.current='';setSelected('');setPanelMode('root');setPageKey('home');setMsg('');if(editorSourceMode==='demo'){demoSnapshotReadyRef.current=false;setDirty(true);return;}const savedBundleSkin=String(data.design?.general?.themeBundleSkinSlug||'');const savedSectionsSkin=String(data.design?.general?.themeBundleSectionsSkinSlug||'');const editPersisted=!!target.current&&skinSlug===savedSkinSlug&&savedBundleSkin===skinSlug&&savedSectionsSkin===skinSlug;if(editPersisted)return;const source=sourceFor(target,skinSlug);designRef.current=source.design;sectionsRef.current=source.sections;setDesign(source.design);setSections(source.sections);setDirty(!target.current||skinSlug!==savedSkinSlug);sendDraft({kind:'design',design:source.design});sendDraft({kind:'sections',sections:source.sections})},[editorMode,themes,editorThemeId,editorSkinParam,editorSourceMode]);
  ...
  async function saveAll(){
    ...
    try{
      ...
      const nextDesign={...design,general:{...(design?.general||{}),skinSlug:targetSkin,skin:targetSkin,themeBundleSkinSlug:targetSkin,themeBundleSectionsSkinSlug:targetSkin,themeBundleVersion:'v22.8.6'}};
