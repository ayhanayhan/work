...
export default function DesignViewRouter({page,data,sid,req,reload}:Props){
  ...
  function storefrontOrigin(){
    const rawDomain=String(data?.store?.domain||'').trim();
    if(rawDomain){
      const value=/^https?:\/\//i.test(rawDomain)?rawDomain:`https://${rawDomain}`;
      try{return new URL(value).origin}catch{}
    }
    const rawSlug=String(data?.store?.publicSlug||data?.store?.slug||'').trim().toLowerCase();
    if(rawSlug){
      const host=rawSlug.includes('.')?rawSlug:`${rawSlug}.ticarti.com`;
      try{return new URL(`https://${host}`).origin}catch{}
    }
    ...
    return blocked?'https://ticarti.com':window.location.origin;
  }
  ...
  async function saveAll(){
    ...
    try{
      ...
      const currentRows=await req('/admin/design/sections?storeId='+encodeURIComponent(sid)+'&pageKey=home');
      for(const row of arr(currentRows)){if(row?.id)await req('/admin/design/sections/'+encodeURIComponent(String(row.id))+'?storeId='+encodeURIComponent(sid),{method:'DELETE'});}
      for(const row of sectionPayload){await req('/admin/design/sections',{method:'POST',body:JSON.stringify({storeId:sid,pageKey:'home',sectionType:row.sectionType,sortOrder:row.sortOrder,enabled:row.enabled,settings:row.settings})});}
  ...
  return <div className="design-editor-v14 design-editor-v15">
    <div className="design-v14-topbar">
      ...
      <div className="design-v14-topbar-right">
        ...
        <button type="button" className="design-v14-top-btn" onClick={()=>window.open(storefrontOrigin()+'/','_blank','noopener,noreferrer')}><Icon icon="iconoir:open-new-window"/> Vitrin</button>
