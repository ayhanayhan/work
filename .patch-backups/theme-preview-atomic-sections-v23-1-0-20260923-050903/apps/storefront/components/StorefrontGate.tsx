...
function PreviewSnapshotReporter({enabled,design,sections,menus}:{enabled:boolean;design:any;sections:any[];menus:any[]}){
 useEffect(()=>{if(!enabled||typeof window==='undefined'||window.parent===window)return;const snapshot={design:clone(design),sections:clone(sections),menus:clone(menus)};const timer=window.setTimeout(()=>window.parent.postMessage({source:'commerce-design-preview',snapshot},'*'),0);return()=>window.clearTimeout(timer)},[enabled,design,sections,menus]);
 return null;
}
...
export default function StorefrontGate({children}:{children:React.ReactNode}){
 ...
 return <StorefrontThemeProvider value={contextValue}><PreviewSnapshotReporter enabled={preview.isPreview&&preview.sourceMode==='demo'} design={design} sections={sections} menus={menus}/><LiveTrackerBeacon/><DesignPreviewBridge/><TrackingScripts settings={boot.store?.settings||{}} store={boot.store}/><ThemeShell boot={effectiveBoot} design={design}>{children}</ThemeShell>{boot.features?.aiAssistant&&<a className="store-ai-launcher" href="/account?ai=1" aria-label="AI"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2.5l1.45 4.05L17.5 8l-4.05 1.45L12 13.5l-1.45-4.05L6.5 8l4.05-1.45L12 2.5Z"/></svg></a>}<CookieBanner/></StorefrontThemeProvider>;
}
