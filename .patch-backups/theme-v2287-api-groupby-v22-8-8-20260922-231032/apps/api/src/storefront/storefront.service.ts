...
export class StorefrontService {
  ...
  async bootstrap(slug: string, currency?: string, locale?: string) {
    ...
    let bestSelling:any[]=[];
    try{
      const sales:any[]=await this.prisma.orderItem.groupBy({by:['variantId'],where:{variantId:{not:null},order:{storeId:store.id}},_sum:{quantity:true},orderBy:{_sum:{quantity:'desc'}},take:80});
