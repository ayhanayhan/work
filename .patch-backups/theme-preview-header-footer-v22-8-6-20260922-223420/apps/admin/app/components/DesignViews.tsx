...
export default function DesignViewRouter({page,data,sid,req,reload}:Props){
  ...
  useEffect(()=>{
    const onPreviewMessage=(event:MessageEvent)=>{
      ...
      if(payload.ready){pushFullDraft();return;}
  ...
  useEffect(()=>{if(!editorMode||!themes.length)return;const target=themes.find((t:any)=>String(t.id)===editorThemeId)||themes.find((t:any)=>t.current)||themes[0];if(!target)return;const skinSlug=String(editorSkinParam||chosenSkin(target));const key=`${target.id}:${skinSlug}:${editorSourceMode}`;if(draftInitRef.current===key)return;draftInitRef.current=key;const savedBundleSkin=String(data.design?.general?.themeBundleSkinSlug||'');const savedSectionsSkin=String(data.design?.general?.themeBundleSectionsSkinSlug||'');const editPersisted=editorSourceMode!=='demo'&&!!target.current&&skinSlug===savedSkinSlug&&savedBundleSkin===skinSlug&&savedSectionsSkin===skinSlug;if(editPersisted)return;const source=sourceFor(target,skinSlug);designRef.current=source.design;sectionsRef.current=source.sections;selectedRef.current='';setDesign(source.design);setSections(source.sections);setSelected('');setPanelMode('root');setPageKey('home');setDirty(editorSourceMode==='demo'||!target.current||skinSlug!==savedSkinSlug);setMsg('');sendDraft({kind:'design',design:source.design});sendDraft({kind:'sections',sections:source.sections})},[editorMode,themes,editorThemeId,editorSkinParam,editorSourceMode]);
  ...
  async function saveAll(){
    ...
    try{
      ...
      const nextDesign={...design,general:{...(design?.general||{}),skinSlug:targetSkin,skin:targetSkin,themeBundleSkinSlug:targetSkin,themeBundleSectionsSkinSlug:targetSkin,themeBundleVersion:'v22.8.1'}};
