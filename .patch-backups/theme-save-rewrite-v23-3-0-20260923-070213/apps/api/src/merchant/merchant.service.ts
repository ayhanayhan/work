...
export class MerchantService {
  ...
  private themeStateOf(store:any){
    const root:any=store?.settings&&typeof store.settings==='object'?store.settings:{};
    const state:any=root.themeState&&typeof root.themeState==='object'&&!Array.isArray(root.themeState)?root.themeState:null;
    return state;
  }
  ...
  async saveThemeEditorSnapshot(tenantId:string,storeId:string,body:any){
    const traceId=`theme-save-${Date.now().toString(36)}-${randomBytes(4).toString('hex')}`;
    let step='start';
    const startedAt=Date.now();
    try{
      step='load-store';
      const store=await this.assertStore(tenantId,storeId);
      step='validate-theme-id';
      const themeId=String(body?.themeId||'').trim();
      if(!themeId)throw new BadRequestException({message:'Tema gerekli',step,traceId});
      step='load-theme-definition';
      const theme=await this.prisma.themeDefinition.findFirst({where:{id:themeId,isActive:true}});
      if(!theme)throw new NotFoundException({message:'Tema bulunamadı',step,traceId,themeId});
      step='load-theme-catalog';
      const catalog=await this.themeCatalog(tenantId,storeId);
      const item:any=catalog.find((x:any)=>String(x.id)===themeId);
      if(!item)throw new NotFoundException({message:'Tema katalogda bulunamadı',step,traceId,themeId});
      if(!item.owned&&Number(item.price)>0)throw new BadRequestException({message:'Bu temayı kullanmak için önce satın almalısınız.',step,traceId,themeId});

      step='normalize-payload';
      const requestedSkin=String(body?.skinSlug||body?.design?.general?.skinSlug||body?.design?.general?.skin||'').trim();
      if(!requestedSkin)throw new BadRequestException({message:'Tema skin bilgisi gerekli',step,traceId});
      const incoming:any=body?.design&&typeof body.design==='object'?body.design:{};
      const incomingGeneral:any={...(incoming.general||{})};delete incomingGeneral.themeName;
      const nextDesign:any={...incoming,general:{...incomingGeneral,skinSlug:requestedSkin,skin:requestedSkin,themeBundleSkinSlug:requestedSkin,themeBundleSectionsSkinSlug:requestedSkin,themeBundleVersion:'v23.2.2'}};
      const sections=(Array.isArray(body?.sections)?body.sections:[]).map((x:any,i:number)=>{const sourceId=String(x?.sourceId||x?.settings?.__sourceId||`section-${i}`).trim();return {id:String(x?.id||sourceId||`section-${i}`),sourceId,pageKey:'home',sectionType:String(x?.sectionType||'rich_text'),sortOrder:i,enabled:x?.enabled!==false,settings:{...(x?.settings||{}),__sourceId:sourceId}}});
      const menus=(Array.isArray(body?.menus)?body.menus:[]).map((m:any)=>({name:String(m?.name||m?.handle||'Menü'),handle:String(m?.handle||''),items:Array.isArray(m?.items)?m.items:[]})).filter((m:any)=>m.handle);
      const root:any=store.settings||{};
      const previousState:any=this.themeStateOf(store);
      const themeState:any={version:1,storage:'TENANT_SETTINGS_THEME_STATE',themeId,themeSlug:theme.slug,skinSlug:requestedSkin,design:nextDesign,sections,menus,savedAt:new Date().toISOString()};

      step='transaction-history';
      await this.prisma.$transaction(async tx=>{
        await tx.themeDesignHistory.create({data:{tenantId:storeId,themeSlug:store.activeTheme,reason:'MANUAL_SAVE',label:'Kaydetmeden önce',snapshot:{activeTheme:store.activeTheme,themeState:previousState||null} as any}});
        step='transaction-installation';
        ...
        step='transaction-tenant-theme-state';
        await tx.tenant.update({where:{id:storeId},data:{activeTheme:theme.slug,settings:{...root,themeState}}});
      });
      step='cache-invalidate';
      await this.cache.delPattern('public:site:*');
      await this.cache.purgeTenant(tenantId);
      console.info('[theme-save]',JSON.stringify({traceId,status:'ok',storeId,themeId,skinSlug:requestedSkin,sectionCount:sections.length,menuCount:menus.length,durationMs:Date.now()-startedAt}));
      return {ok:true,saveMode:'THEME_STATE_JSON_V1',debug:{traceId,durationMs:Date.now()-startedAt,sectionCount:sections.length,menuCount:menus.length},activeTheme:theme.slug,themeId,skinSlug:requestedSkin,themeState,design:nextDesign,sections,menus};
    }catch(e:any){
      const status=typeof e?.getStatus==='function'?e.getStatus():500;
      const prismaCode=String(e?.code||'');
      const safeMeta=e?.meta&&typeof e.meta==='object'?e.meta:undefined;
      const originalResponse=typeof e?.getResponse==='function'?e.getResponse():null;
      const originalMessage=typeof originalResponse==='object'&&originalResponse?.message?originalResponse.message:e?.message;
      console.error('[theme-save]',JSON.stringify({traceId,status:'error',step,httpStatus:status,storeId,tenantId,code:prismaCode||undefined,message:String(originalMessage||'Unknown theme save error'),meta:safeMeta,durationMs:Date.now()-startedAt,stack:String(e?.stack||'').split('\n').slice(0,8).join(' | ')}));
      if(status<500&&typeof e?.getStatus==='function')throw e;
      throw new InternalServerErrorException({message:'Tema kaydı başarısız',step,code:prismaCode||undefined,traceId,detail:String(originalMessage||e?.message||'Bilinmeyen hata'),meta:safeMeta});
    }
  }

  async designDebug(tenantId:string,storeId:string){
    const store=await this.assertStore(tenantId,storeId);
    const state:any=this.themeStateOf(store);
    const themeId=String(state?.themeId||'');
    const definition=themeId?await this.prisma.themeDefinition.findUnique({where:{id:themeId},select:{id:true,slug:true,name:true,isActive:true}}):null;
    const installation=themeId?await this.prisma.themeInstallation.findUnique({where:{tenantId_themeId:{tenantId,themeId}},select:{status:true,source:true,activatedAt:true}}):null;
    return {apiMarker:'THEME_SAVE_DEBUG_V23_2_2',storeId,tenantId,activeTheme:store.activeTheme,settingsType:Array.isArray(store.settings)?'array':typeof store.settings,themeStatePresent:!!state,themeState:{storage:state?.storage||null,version:state?.version||null,themeId:state?.themeId||null,themeSlug:state?.themeSlug||null,skinSlug:state?.skinSlug||null,sectionCount:Array.isArray(state?.sections)?state.sections.length:0,menuCount:Array.isArray(state?.menus)?state.menus.length:0,savedAt:state?.savedAt||null},themeDefinition:definition,themeInstallation:installation};
  }
  ...
  async reorderDesignSections(tenantId:string,storeId:string,body:any){const current=await this.designSections(tenantId,storeId,'home');const order=new Map((Array.isArray(body?.items)?body.items:[]).map((x:any,i:number)=>[String(x.id),Number(x.sortOrder??i)]));const next=[...current].sort((a:any,b:any)=>Number(order.get(String(a.id||a.sourceId))??a.sortOrder??0)-Number(order.get(String(b.id||b.sourceId))??b.sortOrder??0));return this.replaceDesignSections(tenantId,storeId,{sections:next});}
  ...
  async themeCatalog(tenantId:string,storeId:string){
    ...
    const rawThemes=await this.prisma.themeDefinition.findMany({where:{isActive:true},include:{planPrices:sub?.planId?{where:{planId:sub.planId,isActive:true}}:false,installations:{where:{tenantId},take:1}},orderBy:[{sortOrder:'asc'},{name:'asc'}]});const themes:any[]=[];for(const row of rawThemes)themes.push(await this.ensureThemeSourceDb(row));
    const state:any=this.themeStateOf(store);const savedSkin=String(state?.skinSlug||state?.design?.general?.skinSlug||'');
    return themes.map((theme:any)=>{const pp=Array.isArray(theme.planPrices)?theme.planPrices[0]:null;const owned=!!theme.installations?.[0];const included=!!pp?.included;const price=included?0:Number(pp?.price??theme.basePrice??0);const sourceTheme=this.isTicartiSourceTheme(theme.slug);const skins=sourceTheme?this.themeSkinRowsFromDb(theme.config):[];const activeSkin=savedSkin||String((theme.config as any)?.defaultSkin||skins[0]?.slug||'');const active=skins.find((x:any)=>x.slug===activeSkin)||skins[0]||null;const config:any=sourceTheme?{...(theme.config||{}),engine:'ticarti-db-theme-v1',defaultSkin:String((theme.config as any)?.defaultSkin||skins[0]?.slug||''),activeSkin,skins,design:active?.design||{},homePreset:active?.homePreset||[]}:theme.config;return {id:theme.id,slug:theme.slug,name:theme.name,description:theme.description,category:theme.category,previewImageUrl:theme.previewImageUrl,version:theme.version,isFeatured:theme.isFeatured,isDefault:theme.isDefault,current:store.activeTheme===theme.slug,owned:owned||included,included,price,currency:pp?.currency||theme.currency,billingType:theme.billingType,config};});
  }

  private async captureThemeDesignHistory(tenantId:string,storeId:string,reason='MANUAL_SAVE',label?:string){
    const store=await this.assertStore(tenantId,storeId);const state:any=this.themeStateOf(store);const snapshot={activeTheme:store.activeTheme,themeState:state||null};
  ...
  async restoreDesignHistory(tenantId:string,storeId:string,id:string){
    ...
    const snap:any=row.snapshot||{};const root:any=store.settings||{};await this.prisma.tenant.update({where:{id:storeId},data:{activeTheme:String(snap.activeTheme||row.themeSlug||store.activeTheme),settings:{...root,themeState:snap.themeState||null}}});
  ...
  async activateTheme(tenantId:string,storeId:string,themeId:string,options:any={}){
    const store=await this.assertStore(tenantId,storeId);const theme:any=await this.prisma.themeDefinition.findFirst({where:{id:themeId,isActive:true}});if(!theme)throw new NotFoundException('Tema bulunamadı');
