...
export default function DesignViewRouter({page,data,sid,req,reload}:Props){
  ...
  function pushFullDraft(){const frame=previewRef.current?.contentWindow;if(!frame)return;frame.postMessage({source:'commerce-design-editor',action:'draft',kind:'design',design:designRef.current},'*');frame.postMessage({source:'commerce-design-editor',action:'draft',kind:'sections',sections:sectionsRef.current},'*');frame.postMessage({source:'commerce-design-editor',action:'draft',kind:'menus',menus:menusRef.current},'*');frame.postMessage({source:'commerce-design-editor',action:'select',target:selectedRef.current},'*')}
  ...
  async function saveAll(){
    ...
    try{
      ...
      const result=await req('/admin/design/settings',{method:'PATCH',body:JSON.stringify({storeId:sid,themeEditorSave:true,themeId:String(targetTheme.id),skinSlug:targetSkin,pageKey:'home',design:nextDesign,sections:sectionPayload,menus})});
      if(!result?.ok||result?.saveMode!=='THEME_STATE_JSON_V1')throw new Error(`Tema kaydı beklenen formatta tamamlanmadı.\nExpected: THEME_STATE_JSON_V1\nReceived: ${String(result?.saveMode||'boş')}\nResponse: ${JSON.stringify(result).slice(0,2000)}`);
      const savedDesign=clone(result.design||nextDesign);
      const savedSections=clone(arr(result.sections));
      const savedMenus=clone(arr(result.menus));
      ...
      let debug:any=null;
      try{debug=await req('/admin/design/debug?storeId='+encodeURIComponent(sid))}catch{}
      const parts=[e?.message||'Tasarım kaydedilemedi.',debug?`\nDB Debug:\n${JSON.stringify(debug,null,2)}`:''];
  ...
  return <div className="design-editor-v14 design-editor-v15">
    ...
    {(msg||err)&&<div className={'design-v14-notice '+(err?'error':'success')} style={err?{whiteSpace:'pre-wrap',fontFamily:'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace',fontSize:12}:undefined}>{err||msg}</div>}
