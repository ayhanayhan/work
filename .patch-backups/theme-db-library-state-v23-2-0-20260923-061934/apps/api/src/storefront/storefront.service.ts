...
export class StorefrontService {
  ...
  async getCart(token:string){const cart=await this.prisma.cart.findUnique({where:{token},include:{items:{include:{variant:{include:{product:{include:{images:{orderBy:{sortOrder:'asc'},take:1},categories:true}},inventory:true}}}},store:true,customer:true}});if(!cart)throw new NotFoundException('Cart not found');await this.prisma.cart.update({where:{id:cart.id},data:{lastActivityAt:new Date(),...(cart.abandonedAt?{recoveredAt:new Date(),abandonedAt:null}:{})}});const totals=await this.calculateCart(cart);const ci=await this.currencyInfo(cart.store,cart.currency);const displayItems=cart.items.map((i:any)=>({...i,variant:{...i.variant,basePrice:Number(i.variant.price),price:this.displayVariantPrice(i.variant.product,i.variant,ci,cart.customer?.groupId)}}));
    const relationIds=new Set<string>();const bundleOffers:any[]=[];
    for(const item of cart.items as any[]){const cfg:any=(item.variant?.product?.metadata as any)?.together||{};for(const id of Array.isArray(cfg.productIds)?cfg.productIds:[])if(String(id)!==String(item.variant?.productId))relationIds.add(String(id));if(cfg.bundleEnabled&&Array.isArray(cfg.productIds)&&cfg.productIds.length)bundleOffers.push({anchorProductId:item.variant.productId,productIds:cfg.productIds.map(String),bundlePrice:cfg.bundlePrice==null?null:this.money(cfg.bundlePrice,ci.rate,ci.rounding)});}
    const existingProductIds=new Set((cart.items as any[]).map((x:any)=>String(x.variant?.productId||'')));const ids=[...relationIds].filter(id=>!existingProductIds.has(id));const related=ids.length?await this.prisma.product.findMany({where:{id:{in:ids},storeId:cart.storeId,status:'ACTIVE'},include:{images:{orderBy:{sortOrder:'asc'},take:1},variants:{where:{isActive:true},include:{inventory:true},take:1}}}):[];const togetherSuggestions=related.map((p:any)=>this.presentProduct(p,ci));
    return{...cart,items:displayItems,totals,promotionGifts:totals.gifts,togetherSuggestions,bundleOffers};}
