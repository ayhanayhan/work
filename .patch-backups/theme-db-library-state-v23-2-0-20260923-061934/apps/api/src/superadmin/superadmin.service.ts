...
export class SuperAdminService {
  ...
  async themeSkinDefaults(themeId:string){
    ...
    const config:any=theme.config&&typeof theme.config==='object'&&!Array.isArray(theme.config)?theme.config:{};const overrides:any=config.skinDefaults&&typeof config.skinDefaults==='object'&&!Array.isArray(config.skinDefaults)?config.skinDefaults:{};
    return ticartiSkinCatalog().map((skin:any)=>{const row:any=overrides[skin.slug]||{};return {slug:skin.slug,name:String(row.name||skin.name),category:String(row.category||skin.category||''),verifiedFromDemo:!!skin.verifiedFromDemo,overridden:!!overrides[skin.slug],design:row.design||ticartiSourceDesignForSkin(skin.slug),homePreset:Array.isArray(row.homePreset)?row.homePreset:ticartiSourceHomePreset(skin.slug)};});
  }
  async updateThemeSkinDefaults(themeId:string,skinValue:string,body:any){
    const theme:any=await this.prisma.themeDefinition.findUnique({where:{id:themeId}});if(!theme)throw new NotFoundException('Tema bulunamadı');
    const slug=normalizeTicartiSkinSlug(skinValue);const base=getTicartiSkinPreset(slug);if(!base)throw new NotFoundException('Skin bulunamadı');
    const config:any=theme.config&&typeof theme.config==='object'&&!Array.isArray(theme.config)?theme.config:{};const overrides:any=config.skinDefaults&&typeof config.skinDefaults==='object'&&!Array.isArray(config.skinDefaults)?{...config.skinDefaults}:{};
    const current:any=overrides[slug]||{};const next:any={...current};
    if(body.name!==undefined)next.name=String(body.name||base.name);if(body.category!==undefined)next.category=String(body.category||base.category||'');
    if(body.design!==undefined){if(!body.design||typeof body.design!=='object'||Array.isArray(body.design))throw new BadRequestException('design geçerli bir JSON nesnesi olmalı');next.design=body.design;}
    if(body.homePreset!==undefined){if(!Array.isArray(body.homePreset))throw new BadRequestException('homePreset bir JSON dizisi olmalı');next.homePreset=body.homePreset.map((x:any,i:number)=>{const sourceId=String(x?.sourceId||x?.settings?.__sourceId||`${slug}-${i}`);return {...x,sourceId,settings:{...(x?.settings||{}),__sourceId:sourceId}}});}
    overrides[slug]=next;const updated=await this.prisma.themeDefinition.update({where:{id:themeId},data:{config:{...config,skinDefaults:overrides}}});return {ok:true,slug,config:updated.config};
  }
  async resetThemeSkinDefaults(themeId:string,skinValue:string){
    const theme:any=await this.prisma.themeDefinition.findUnique({where:{id:themeId}});if(!theme)throw new NotFoundException('Tema bulunamadı');const slug=normalizeTicartiSkinSlug(skinValue);getTicartiSkinPreset(slug);
    const config:any=theme.config&&typeof theme.config==='object'&&!Array.isArray(theme.config)?theme.config:{};const overrides:any=config.skinDefaults&&typeof config.skinDefaults==='object'&&!Array.isArray(config.skinDefaults)?{...config.skinDefaults}:{};delete overrides[slug];
    await this.prisma.themeDefinition.update({where:{id:themeId},data:{config:{...config,skinDefaults:overrides}}});return {ok:true,slug};
  }
