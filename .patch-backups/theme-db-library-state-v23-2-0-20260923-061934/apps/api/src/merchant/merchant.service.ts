...
export class MerchantService {
  ...
  async saveThemeEditorSnapshot(tenantId:string,storeId:string,body:any){
    const store=await this.assertStore(tenantId,storeId);
    const themeId=String(body?.themeId||'').trim();
    if(!themeId)throw new BadRequestException('Tema gerekli');
    const theme=await this.prisma.themeDefinition.findFirst({where:{id:themeId,isActive:true}});
    if(!theme)throw new NotFoundException('Tema bulunamadı');
    const catalog=await this.themeCatalog(tenantId,storeId);
    const item:any=catalog.find((x:any)=>String(x.id)===themeId);
    if(!item)throw new NotFoundException('Tema bulunamadı');
    if(!item.owned&&Number(item.price)>0)throw new BadRequestException('Bu temayı kullanmak için önce satın almalısınız.');

    const requestedSkin=normalizeTicartiSkinSlug(body?.skinSlug||body?.design?.general?.skinSlug||body?.design?.general?.skin||TICARTI_DEFAULT_SKIN);
    const incoming:any=body?.design&&typeof body.design==='object'?body.design:{};
    const incomingGeneral:any={...(incoming.general||{})};
    delete incomingGeneral.themeName;
    const nextDesign:any={
      ...incoming,
      general:{
        ...incomingGeneral,
        skinSlug:requestedSkin,
        skin:requestedSkin,
        themeBundleSkinSlug:requestedSkin,
        themeBundleSectionsSkinSlug:requestedSkin,
        themeBundleVersion:'v23.1.2'
      }
    };
    const pageKey=String(body?.pageKey||'home');
    const items=Array.isArray(body?.sections)?body.sections:[];
    const normalizedSections=items.map((x:any,i:number)=>{
      const sourceId=String(x?.sourceId||x?.settings?.__sourceId||'').trim();
      return {
        storeId,
        pageKey,
        ...
        settings:{...(x?.settings||{}),...(sourceId?{__sourceId:sourceId}:{})}
      };
    });
    const menus=Array.isArray(body?.menus)?body.menus:[];
    const normalizedMenus=menus
      .map((m:any)=>({
        name:String(m?.name||m?.handle||'Menü').trim()||'Menü',
    ...
    await this.prisma.$transaction(async tx=>{
      const currentSections=await tx.siteSection.findMany({where:{storeId,pageKey},orderBy:{sortOrder:'asc'}});
      await tx.themeDesignHistory.create({data:{
        tenantId:storeId,
        themeSlug:store.activeTheme,
        reason:'MANUAL_SAVE',
        label:'Kaydetmeden önce',
        snapshot:{
          activeTheme:store.activeTheme,
          design:(store.settings as any)?.design||{},
          sections:currentSections.map((x:any)=>({pageKey:x.pageKey,sectionType:x.sectionType,sortOrder:x.sortOrder,enabled:x.enabled,settings:x.settings}))
        } as any
      }});

      await tx.themeInstallation.upsert({
        where:{tenantId_themeId:{tenantId,themeId}},
        create:{tenantId,themeId,status:'ACTIVE',source:item.included?'PLAN':'FREE',purchasedPrice:0,currency:item.currency,activatedAt:new Date()},
        update:{status:'ACTIVE',activatedAt:new Date()}
      });

      const root:any=store.settings||{};
      await tx.tenant.update({where:{id:storeId},data:{activeTheme:theme.slug,settings:{...root,design:nextDesign}}});
      await tx.siteSection.deleteMany({where:{storeId,pageKey}});
      if(normalizedSections.length)await tx.siteSection.createMany({data:normalizedSections});

      for(const menu of normalizedMenus){
        await tx.menu.upsert({
          where:{storeId_handle:{storeId,handle:menu.handle}},
          create:{storeId,name:menu.name,handle:menu.handle,items:menu.items},
          update:{name:menu.name,items:menu.items}
        });
      }
    });
    ...
    const [design,sections,savedMenus]=await Promise.all([
      this.designSettings(tenantId,storeId),
      this.designSections(tenantId,storeId,pageKey),
      this.menus(tenantId,storeId)
    ]);
    return {ok:true,saveMode:'ATOMIC_THEME_EDITOR_V23_1_2',activeTheme:theme.slug,themeId,skinSlug:requestedSkin,design,sections,menus:savedMenus};
  }
  ...
  async replaceDesignSections(tenantId:string,storeId:string,body:any){
    await this.assertStore(tenantId,storeId);const pageKey=String(body?.pageKey||'home');const items=Array.isArray(body?.items)?body.items:[];
    const normalized=items.map((x:any,i:number)=>{const sourceId=String(x?.sourceId||x?.settings?.__sourceId||'').trim();return {storeId,pageKey,sectionType:String(x?.sectionType||'rich_text'),sortOrder:i,enabled:x?.enabled!==false,settings:{...(x?.settings||{}),...(sourceId?{__sourceId:sourceId}:{})}}});
    await this.prisma.$transaction(async tx=>{await tx.siteSection.deleteMany({where:{storeId,pageKey}});if(normalized.length)await tx.siteSection.createMany({data:normalized});});await this.cache.delPattern('public:site:*');await this.cache.purgeTenant(tenantId);return this.designSections(tenantId,storeId,pageKey);
  }
  ...
  async refreshCurrencyRates(tenantId:string,storeId:string,body:any){
    ...
    for(const row of rows){const pinned=!!regional.currencyMeta?.[row.code]?.pinned;const rate=row.code===base?1:Number(rates[row.code]||0);if(rate>0&&!pinned)await this.prisma.storeCurrency.update({where:{id:row.id},data:{exchangeRate:rate}});}
  ...
  private isTicartiSourceTheme(slug:any){return ['nova-commerce','ticarti','signature','wokiee'].includes(String(slug||'').toLowerCase());}
  private ticartiSourceOverride(config:any,skinValue:any){
    const slug=normalizeTicartiSkinSlug(skinValue);const overrides:any=config?.skinDefaults&&typeof config.skinDefaults==='object'&&!Array.isArray(config.skinDefaults)?config.skinDefaults:{};const row:any=overrides[slug]||{};
    const design:any=row.design&&typeof row.design==='object'&&!Array.isArray(row.design)?row.design:ticartiSourceDesignForSkin(slug);
    const homePreset:any[]=Array.isArray(row.homePreset)?row.homePreset:ticartiSourceHomePreset(slug);
    return {slug,design:{...design,general:{...(design.general||{}),themeId:'ticarti',themePackageId:'ticarti',skinSlug:slug,skin:slug}},homePreset:homePreset.map((x:any,i:number)=>({sourceId:String(x.sourceId||x.settings?.__sourceId||`${slug}-${i}`),sectionType:String(x.sectionType||'rich_text'),sortOrder:Number(x.sortOrder??i),enabled:x.enabled!==false,settings:{...(x.settings||{}),__sourceId:String(x.sourceId||x.settings?.__sourceId||`${slug}-${i}`)}}))};
  }
  private ticartiCatalogWithOverrides(config:any){
    const overrides:any=config?.skinDefaults&&typeof config.skinDefaults==='object'&&!Array.isArray(config.skinDefaults)?config.skinDefaults:{};
    return ticartiSkinCatalog().map((skin:any)=>{const row:any=overrides[skin.slug]||{};const source=this.ticartiSourceOverride(config,skin.slug);return {...skin,...(row.name?{name:String(row.name)}:{}),...(row.category?{category:String(row.category)}:{}),...(row.previewImageUrl?{previewImageUrl:String(row.previewImageUrl)}:{}),design:source.design,homePreset:source.homePreset,sourceManagedBy:'SUPERADMIN'};});
  }
  async themeCatalog(tenantId:string,storeId:string){
    ...
    const savedDesign:any=(store.settings as any)?.design||{};const savedSkin=normalizeTicartiSkinSlug(savedDesign?.general?.skinSlug||savedDesign?.general?.skin||TICARTI_DEFAULT_SKIN);
    return themes.map((theme:any)=>{const pp=Array.isArray(theme.planPrices)?theme.planPrices[0]:null;const owned=!!theme.installations?.[0];const included=!!pp?.included;const price=included?0:Number(pp?.price??theme.basePrice??0);const sourceTheme=this.isTicartiSourceTheme(theme.slug);const source=sourceTheme?this.ticartiSourceOverride(theme.config,savedSkin):null;const config:any=sourceTheme?{...(theme.config||{}),engine:'ticarti-skins-v3',defaultSkin:TICARTI_DEFAULT_SKIN,activeSkin:savedSkin,skins:this.ticartiCatalogWithOverrides(theme.config),design:source?.design,homePreset:source?.homePreset}:theme.config;return {id:theme.id,slug:theme.slug,name:sourceTheme?'Ticarti':theme.name,description:theme.description,category:theme.category,previewImageUrl:theme.previewImageUrl,version:theme.version,isFeatured:theme.isFeatured,isDefault:theme.isDefault,current:store.activeTheme===theme.slug,owned:owned||included,included,price,currency:pp?.currency||theme.currency,billingType:theme.billingType,config};});
  }
  ...
  async activateTheme(tenantId:string,storeId:string,themeId:string,options:any={}){
    ...
    const isTicartiTheme=['nova-commerce','ticarti','signature','wokiee'].includes(String(theme.slug||'').toLowerCase());
    const root:any=store.settings||{};const currentDesign:any=await this.designSettings(tenantId,storeId);
    const requestedSkin=normalizeTicartiSkinSlug(options?.skinSlug||options?.skin||currentDesign?.general?.skinSlug||currentDesign?.general?.skin||cfg?.activeSkin||cfg?.defaultSkin||TICARTI_DEFAULT_SKIN);
    const resolvedSource=isTicartiTheme?this.ticartiSourceOverride(cfg,requestedSkin):null;
    const preset:any[]=isTicartiTheme?(resolvedSource?.homePreset||[]):(Array.isArray(cfg.homePreset)?cfg.homePreset:[]);
    const themeDesign:any=isTicartiTheme?(resolvedSource?.design||{}):(cfg.design||{});
    const preserveThemeSettings=options?.preserveThemeSettings===true;
    const loadThemeModules=options?.loadThemeModules!==false;
    const sourceThemeSettings:any=themeDesign.themeSettings||{};const currentThemeSettings:any=currentDesign.themeSettings||{};
    const nextDesign:any=preserveThemeSettings?{
      ...themeDesign,...currentDesign,
      general:{...(themeDesign.general||{}),...currentDesign.general,skinSlug:requestedSkin,skin:requestedSkin,themeBundleSkinSlug:requestedSkin,themeBundleVersion:'v22.8.1',...(loadThemeModules?{themeBundleSectionsSkinSlug:requestedSkin}:{})},
      header:{...(themeDesign.header||{}),...currentDesign.header},footer:{...(themeDesign.footer||{}),...currentDesign.footer},products:{...(themeDesign.products||{}),...currentDesign.products},
      themeSettings:{...sourceThemeSettings,...currentThemeSettings,general:{...(sourceThemeSettings.general||{}),...(currentThemeSettings.general||{})},header:{...(sourceThemeSettings.header||{}),...(currentThemeSettings.header||{})},footer:{...(sourceThemeSettings.footer||{}),...(currentThemeSettings.footer||{})},products:{...(sourceThemeSettings.products||{}),...(currentThemeSettings.products||{})}}
    }:{
      ...themeDesign,
      general:{...(themeDesign.general||{}),skinSlug:requestedSkin,skin:requestedSkin,themeBundleSkinSlug:requestedSkin,themeBundleVersion:'v22.8.1',...(loadThemeModules?{themeBundleSectionsSkinSlug:requestedSkin}:{})},
      header:{...(themeDesign.header||{})},footer:{...(themeDesign.footer||{})},products:{...(themeDesign.products||{})},
      themeSettings:{...sourceThemeSettings,general:{...(sourceThemeSettings.general||{})},header:{...(sourceThemeSettings.header||{})},footer:{...(sourceThemeSettings.footer||{})},products:{...(sourceThemeSettings.products||{})}}
    };
    ...
    await this.prisma.$transaction(async tx=>{
      ...
      if(loadThemeModules){
        ...
        if(preset.length)await tx.siteSection.createMany({data:preset.map((x:any,i:number)=>({storeId,pageKey:'home',sectionType:String(x.sectionType||'rich_text'),sortOrder:Number(x.sortOrder??i),enabled:x.enabled!==false,settings:{...(x.settings||{}),...(x.sourceId?{__sourceId:x.sourceId}:{})}}))});
      }else if(currentSections.length&&preset.length){
        const used=new Set<string>();let order=0;
        for(const p of preset){const sourceId=String(p.sourceId||p.settings?.__sourceId||'');const match=currentSections.find((x:any)=>!used.has(x.id)&&sourceId&&String((x.settings as any)?.__sourceId||'')===sourceId);if(!match)continue;used.add(match.id);const keep:any={};for(const key of contentKeys)if((match.settings as any)?.[key]!==undefined)keep[key]=(match.settings as any)[key];await tx.siteSection.update({where:{id:match.id},data:{sectionType:String(p.sectionType||match.sectionType),sortOrder:order++,enabled:p.enabled!==false,settings:{...(p.settings||{}),...keep,__sourceId:sourceId}}});}
        for(const x of currentSections)if(!used.has(x.id))await tx.siteSection.update({where:{id:x.id},data:{sortOrder:order++}});
      }
    });
    await this.cache.delPattern('public:site:*');await this.cache.purgeTenant(tenantId);return {activeTheme:theme.slug,skinSlug:requestedSkin,loadThemeModules,design:nextDesign,sections:await this.designSections(tenantId,storeId,'home')};
  }
