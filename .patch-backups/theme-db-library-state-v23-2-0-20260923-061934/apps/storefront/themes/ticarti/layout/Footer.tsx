...
export default function Footer({boot}:{boot:any}){
  ...
  const store=boot?.store||{};const design=themeState.design||boot?.design||{};const skin=resolveTicartiSkin(design);const sourceFooter=skin?.design?.footer||{};const footer={...sourceFooter,...(design.footer||{})};
  ...
  <footer id={footerId} className={`footer top-spacing-none ${String(footer.layoutClass||'')} ${sectionClass} section-background section-background--medium`} data-section-name="wokiee-footer" data-theme-skin={skin.slug} style={{'--color-section-background':footer.backgroundColor||footer.color_background||undefined} as any}>
