...
function withBundleMarker(design:any,skinSlug:string){const next=clone(design||{});next.general={...(next.general||{}),skinSlug,skin:skinSlug,themeBundleSkinSlug:skinSlug,themeBundleVersion:'v22.9.0'};return next}
...
function normalizeDesign(raw:any,{trustStore=false}:{trustStore?:boolean}={}){
 const skin=resolveTicartiSkin(raw||{});
 const source=withBundleMarker(skin?.design||{},skin.slug);
 const marker=String(raw?.general?.themeBundleSkinSlug||raw?.general?.skinSlug||raw?.general?.skin||'');
 const candidate=(trustStore||marker===skin.slug)?(raw||{}):{};
 const merged:any=mergeSkinDesign(candidate,skin);
 const themeName=String(candidate?.general?.themeName||merged?.general?.themeName||source?.general?.themeName||'Ticarti');
 return {
   ...source,...merged,...candidate,
   general:{...(source.general||{}),...(merged.general||{}),...(candidate.general||{}),themeName,skinSlug:skin.slug,skin:skin.slug,themeBundleSkinSlug:skin.slug,themeBundleVersion:'v23.0.8'},
   header:{...(source.header||{}),...(merged.header||{}),...(candidate.header||{})},
   footer:{...(source.footer||{}),...(merged.footer||{}),...(candidate.footer||{})},
   products:{...(source.products||{}),...(merged.products||{}),...(candidate.products||{})},
   themeSettings:{
     ...(source.themeSettings||{}),...(merged.themeSettings||{}),...(candidate.themeSettings||{}),
     general:{...(source.themeSettings?.general||{}),...(merged.themeSettings?.general||{}),...(candidate.themeSettings?.general||{})},
     header:{...(source.themeSettings?.header||{}),...(merged.themeSettings?.header||{}),...(candidate.themeSettings?.header||{})},
     footer:{...(source.themeSettings?.footer||{}),...(merged.themeSettings?.footer||{}),...(candidate.themeSettings?.footer||{})},
     products:{...(source.themeSettings?.products||{}),...(merged.themeSettings?.products||{}),...(candidate.themeSettings?.products||{})},
   },
 };
}
function sourceId(row:any){return String(row?.sourceId||row?.settings?.__sourceId||'').trim()}

function PreviewSnapshotReporter({enabled,design,sections,menus,skinSlug}:{enabled:boolean;design:any;sections:any[];menus:any[];skinSlug:string}){
 useEffect(()=>{if(!enabled||typeof window==='undefined'||window.parent===window)return;const snapshot={design:clone(design),sections:clone(sections),menus:clone(menus),skinSlug};const timer=window.setTimeout(()=>window.parent.postMessage({source:'commerce-design-preview',snapshot},'*'),0);return()=>window.clearTimeout(timer)},[enabled,design,sections,menus,skinSlug]);
 return null;
}
...
export default function StorefrontGate({children}:{children:React.ReactNode}){
 ...
 const design=normalizeDesign(rawDesign,{trustStore:preview.sourceMode!=='demo'});
 ...
 const savedLooksLikeCurrentBundle=sectionMarker===skin.slug&&savedSections.length>0;
 ...
 else if(savedSections.length){sections=savedSections;sectionsSource=savedLooksLikeCurrentBundle?'store-bundle':'store-bundle-legacy-marker'}
 ...
 const contextValue={boot:effectiveBoot,design,sections,menus,isDesignPreview:preview.isPreview,sourceMode:preview.sourceMode,debug:{renderMode:preview.sourceMode==='demo'?'source-bundle':'store-bundle',designSource:preview.sourceMode==='demo'?'source-demo':'store-bundle',sectionsSource,bootSectionCount:savedSections.length,savedSourceMatchCount,bootSections:savedSections,expectedFirstSource:sourceId(sourceSections[0]),expectedFirstTitle:String(sourceSections[0]?.settings?.main_heading||sourceSections[0]?.settings?.title||'')}};
 return <StorefrontThemeProvider value={contextValue}><PreviewSnapshotReporter enabled={preview.isPreview&&preview.sourceMode==='demo'} design={design} sections={sections} menus={menus} skinSlug={skin.slug}/><LiveTrackerBeacon/><DesignPreviewBridge/><TrackingScripts settings={boot.store?.settings||{}} store={boot.store}/><ThemeShell boot={effectiveBoot} design={design}>{children}</ThemeShell>{boot.features?.aiAssistant&&<a className="store-ai-launcher" href="/account?ai=1" aria-label="AI"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2.5l1.45 4.05L17.5 8l-4.05 1.45L12 13.5l-1.45-4.05L6.5 8l4.05-1.45L12 2.5Z"/></svg></a>}<CookieBanner/></StorefrontThemeProvider>;
}
