...
export default function DesignViewRouter({page,data,sid,req,reload}:Props){
  ...
  const themesPanel=<>
    <SidebarSubHeader title="Tema Kataloğu" subtitle="Paketinize dahil skinlerden mağaza görünümünü seçin" onBack={panelBack}/>
    <div className="design-v14-tree-scroll">
      {themeErr&&<div className="design-v15-inline-error">{themeErr}</div>}
      <label className="design-theme-import-option"><input type="checkbox" checked={loadThemeModules} onChange={e=>setLoadThemeModules(e.target.checked)}/><span><b>Skin modüllerini ve örnek düzenini yükle</b><small>Açıkken anasayfa seçilen skin düzeniyle yeniden kurulur. Kapalıyken mevcut içerikleriniz korunur ve yalnızca eşleşen modüllerin görünümü uygulanır.</small></span></label>
      <div className="design-theme-list">{themes.flatMap((t:any)=>{const skins=Array.isArray(t.config?.skins)&&t.config.skins.length?t.config.skins:[null];return skins.map((skin:any)=>{const key=String(t.id)+':'+String(skin?.slug||'');const previewing=previewThemeId===key;const current=t.current&&(!skin||String(t.activeSkin||'')===String(skin.slug));const skinDesign=skin?.design||{};const accent=skinDesign.general?.primaryColor||skinDesign.general?.accentColor||t.config?.style?.accent||'#304ffe';const preset=Array.isArray(skin?.homePreset)?skin.homePreset:t.config?.homePreset;const title=skin?.name||t.name;const category=skin?.category||t.category;return <div key={key} className={'design-theme-card '+(current?'active ':'')+(previewing?'previewing':'')}><div className="design-theme-preview" style={{background:`linear-gradient(135deg,${accent}22,#fff)`}}><div className="design-theme-status">{previewing&&<span className="preview-badge"><Icon icon="iconoir:eye"/> Önizleniyor</span>}{current&&<span className="active-badge"><Icon icon="iconoir:check-circle"/> Aktif</span>}</div><span>{category}</span><b>{title}</b><small>{Array.isArray(preset)?`${preset.length} bölüm preset`:'Skin preset'}</small></div><div className="design-theme-meta"><div><b>{title}</b><small>{skin?`${t.name} · ${skin.group==='main-demos'?'Ana demo':skin.group==='classic-skins'?'Klasik skin':'Yeni skin'}`:t.description}</small></div><em>{current?'Aktif':t.included?'Pakete dahil':t.owned?'Satın alındı':Number(t.price)>0?`${Number(t.price).toLocaleString('tr-TR')} ${t.currency}`:'Ücretsiz'}</em></div><div className="design-theme-actions"><button type="button" className="design-v15-light-btn" onClick={()=>previewing?closeThemePreview():previewTheme(t,skin)}>{previewing?'Önizlemeyi Kapat':'Önizle'}</button>{!current&&(t.owned||t.included||Number(t.price)===0)?<button type="button" className="design-v15-primary-btn" onClick={async()=>{try{const out=await req('/admin/themes/'+t.id+'/activate',{method:'POST',body:JSON.stringify({storeId:sid,loadThemeModules,skinSlug:skin?.slug||undefined})});if(out?.design)setDesign(out.design);if(Array.isArray(out?.sections))setSections(out.sections);setPreviewThemeId('');setDirty(false);setThemes(await req('/admin/themes?storeId='+encodeURIComponent(sid)));setHistory(await req('/admin/design/history?storeId='+encodeURIComponent(sid)));await reload()}catch(e:any){setThemeErr(e.message)}}}>Kullan</button>:!current?<button type="button" className="design-v15-primary-btn" onClick={async()=>{try{const out=await req('/admin/billing/iyzico/theme-checkout',{method:'POST',body:JSON.stringify({themeId:t.id})});if(out?.included||out?.owned){setThemes(await req('/admin/themes?storeId='+encodeURIComponent(sid)));return}if(out?.paymentUrl)window.location.href=out.paymentUrl}catch(e:any){setThemeErr(e.message)}}}>Satın al</button>:null}</div></div>})})}</div>
    </div>
  </>;
