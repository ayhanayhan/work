...
export class MerchantService {
  ...
  async saveThemeEditorSnapshot(tenantId:string,storeId:string,body:any){
    ...
    if(!requestedSkin)throw new BadRequestException('Tema skin bilgisi gerekli');
    ...
    const nextDesign:any={...incoming,general:{...incomingGeneral,skinSlug:requestedSkin,skin:requestedSkin,themeBundleSkinSlug:requestedSkin,themeBundleSectionsSkinSlug:requestedSkin,themeBundleVersion:'v23.2.0'}};
    ...
    await this.cache.delPattern('public:site:*');await this.cache.purgeTenant(tenantId);
    return {ok:true,saveMode:'THEME_STATE_JSON_V1',activeTheme:theme.slug,themeId,skinSlug:requestedSkin,themeState,design:nextDesign,sections,menus};
  }
