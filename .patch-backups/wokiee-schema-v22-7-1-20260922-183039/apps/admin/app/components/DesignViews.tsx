...
function SectionSettings({section,update}:{section:any;update:(patch:any)=>void}){const s=section.settings||{};const schema=sectionSchemaFor(section.sectionType);if(!schema)return <LegacySectionSettings section={section} update={update}/>;const blockSchema=(Array.isArray(schema.blocks)?schema.blocks:[]).find((b:any)=>b.type!=='@app')||null;const items=Array.isArray(s.items)?s.items:[];return <>
  ...
  {blockSchema&&<><SectionHead text="Bloklar / içerikler"/><div className="design-v22-block-list">{items.map((item:any,index:number)=><details key={item.id||index} open={index===0}><summary><span>{sourceLabel(blockSchema.name,'Blok')} #{index+1}</span><button type="button" onClick={e=>{e.preventDefault();e.stopPropagation();update({items:items.filter((_:any,i:number)=>i!==index)})}}><Icon icon="iconoir:trash"/></button></summary><SchemaSettingsList settings={blockSchema.settings||[]} values={item} onChange={(key,value)=>{const next=items.map((x:any,i:number)=>i===index?patchSchemaValue(x,key,value):x);update({items:next})}}/></details>)}</div><button type="button" className="design-v14-add-row" onClick={()=>update({items:[...items,{id:`block-${Date.now()}`} ]})}><Icon icon="iconoir:plus"/> Blok ekle</button></>}
</>}
