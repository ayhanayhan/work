...
export default function DesignViewRouter({page,data,sid,req,reload}:Props){
  ...
  useEffect(()=>{
    const onPreviewMessage=(event:MessageEvent)=>{
      ...
      if(payload.snapshot&&editorSourceMode==='demo'){
        ...
        setDesign(sourceDesign);setSections(sourceSections);setMenus(sourceMenus);setDirty(false);setMsg('');
        return;
      }
  ...
  async function saveAll(){
    ...
    try{
      ...
      const nextDesign={...design,general:{...cleanGeneral,skinSlug:targetSkin,skin:targetSkin,themeBundleSkinSlug:targetSkin,themeBundleSectionsSkinSlug:targetSkin,themeBundleVersion:'v23.0.3'}};
      const out=await req('/admin/design/publish',{method:'POST',body:JSON.stringify({storeId:sid,themeId:targetTheme.id,skinSlug:targetSkin,design:nextDesign,pageKey:'home',sections:sections.map((x:any,i:number)=>({sourceId:x.sourceId||x.settings?.__sourceId,sectionType:x.sectionType,sortOrder:i,enabled:x.enabled!==false,settings:x.settings||{}}))})});
      ...
      if(editorSourceMode==='demo'){const url=new URL(window.location.href);url.searchParams.set('sourceMode','edit');url.searchParams.set('themeId',String(targetTheme.id));url.searchParams.set('skinSlug',targetSkin);window.location.replace(url.toString());return;}
