...
export default async function Layout({children}:{children:React.ReactNode}){
  ...
  return <html lang={locale.split('-')[0]||'tr'} dir={rtl?'rtl':'ltr'}><body>{organization&&<script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(organization).replace(/</g,'\\u003c')}}/>}{platform?children:<StorefrontGate>{children}</StorefrontGate>}</body></html>;
}
