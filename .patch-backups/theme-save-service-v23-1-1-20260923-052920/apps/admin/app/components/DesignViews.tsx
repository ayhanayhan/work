...
export default function DesignViewRouter({page,data,sid,req,reload}:Props){
  ...
  useEffect(()=>{
    ...
    if(editorSourceMode==='demo'){demoSnapshotReadyRef.current=false;designRef.current={};sectionsRef.current=[];menusRef.current=[];setDesign({});setSections([]);setMenus([]);setDirty(false);}
  ...
  useEffect(()=>{
    const onPreviewMessage=(event:MessageEvent)=>{
      ...
      if(payload.snapshot&&editorSourceMode==='demo'){
        const snap=payload.snapshot||{};
        const snapSkin=String(snap.skinSlug||'').trim();
        if(editorSkinParam&&snapSkin&&snapSkin!==editorSkinParam)return;
        const sourceDesign=clone(snap.design||{});
        const sourceSections=clone(Array.isArray(snap.sections)?snap.sections:[]);
        const sourceMenus=clone(Array.isArray(snap.menus)?snap.menus:[]);
        demoSnapshotReadyRef.current=true;setPreviewReady(true);
        designRef.current=sourceDesign;sectionsRef.current=sourceSections;menusRef.current=sourceMenus;
        setDesign(sourceDesign);setSections(sourceSections);setMenus(sourceMenus);setDirty(true);setMsg('');
        return;
      }
      if(payload.ready){if(editorSourceMode!=='demo'){pushFullDraft();setPreviewReady(true)}return;}
  ...
  function sourceFor(t:any,skinSlug:string){const skins=Array.isArray(t?.config?.skins)?t.config.skins:[];const skin=skins.find((x:any)=>String(x.slug)===String(skinSlug))||skins[0]||null;const resolvedSlug=String(skin?.slug||skinSlug);const sourceDesign=clone(skin?.design||t?.config?.design||{});sourceDesign.general={...(sourceDesign?.general||{}),skinSlug:resolvedSlug,skin:resolvedSlug,skinName:String(skin?.name||skinSlug),themeBundleSkinSlug:resolvedSlug};const preset=clone(Array.isArray(skin?.homePreset)?skin.homePreset:(Array.isArray(t?.config?.homePreset)?t.config.homePreset:[]));return {skin,design:sourceDesign,sections:preset.map((x:any,i:number)=>{const sourceId=String(x.sourceId||x.settings?.__sourceId||`${skinSlug}-${i}`);return {id:`draft:${sourceId}:${i}`,sourceId,pageKey:'home',sectionType:String(x.sectionType||'rich_text'),sortOrder:i,enabled:x.enabled!==false,settings:{...(x.settings||{}),__sourceId:sourceId}}})}}
  ...
  useEffect(()=>{if(!editorMode||!themes.length)return;const target=themes.find((t:any)=>String(t.id)===editorThemeId)||themes.find((t:any)=>t.current)||themes[0];if(!target)return;const skinSlug=String(editorSkinParam||chosenSkin(target));const key=`${target.id}:${skinSlug}:${editorSourceMode}`;if(draftInitRef.current===key)return;draftInitRef.current=key;selectedRef.current='';setSelected('');setPanelMode('root');setPageKey('home');setMsg('');if(editorSourceMode==='demo'){demoSnapshotReadyRef.current=false;setPreviewReady(false);designRef.current={};sectionsRef.current=[];menusRef.current=[];setDesign({});setSections([]);setMenus([]);setDirty(false);return;}const savedDesign=clone(data.design||{});const savedSections=clone(arr(data.sections));const savedMenus=clone(arr(data.menus));designRef.current=savedDesign;sectionsRef.current=savedSections;menusRef.current=savedMenus;setDesign(savedDesign);setSections(savedSections);setMenus(savedMenus);setDirty(false);queueMicrotask(()=>{sendDraft({kind:'design',design:savedDesign});sendDraft({kind:'sections',sections:savedSections});sendDraft({kind:'menus',menus:savedMenus})})},[editorMode,themes,editorThemeId,editorSkinParam,editorSourceMode,data.design,data.sections,data.menus]);
  ...
  async function saveAll(){
    if(editorSourceMode==='demo'&&!demoSnapshotReadyRef.current){setErr('Tema demosu henüz tamamen yüklenmedi. Lütfen önizleme hazır olduğunda tekrar Kaydet deyin.');return;}
    if(!dirty)return;
    ...
    try{
      ...
      const nextDesign={...design,general:{...cleanGeneral,skinSlug:targetSkin,skin:targetSkin,themeBundleSkinSlug:targetSkin,themeBundleSectionsSkinSlug:targetSkin,themeBundleVersion:'v23.0.9'}};
      ...
      if(!targetTheme.current){await req('/admin/themes/'+encodeURIComponent(String(targetTheme.id))+'/activate',{method:'POST',body:JSON.stringify({storeId:sid,skinSlug:targetSkin,loadThemeModules:false,preserveThemeSettings:false})});}
      await req('/admin/design/settings',{method:'PATCH',body:JSON.stringify({storeId:sid,design:nextDesign})});
      await req('/admin/design/sections/snapshot',{method:'PATCH',body:JSON.stringify({storeId:sid,pageKey:'home',items:sectionPayload})});
      ...
      const [saved,savedSections,nextThemes]=await Promise.all([req('/admin/design/settings?storeId='+encodeURIComponent(sid)),req('/admin/design/sections?storeId='+encodeURIComponent(sid)+'&pageKey=home'),req('/admin/themes?storeId='+encodeURIComponent(sid))]);
      const expected=sectionPayload.map((x:any)=>String(x.settings?.__sourceId||'')).filter(Boolean);
      const actual=arr(savedSections).map((x:any)=>String(x.settings?.__sourceId||'')).filter(Boolean);
      if(sectionPayload.length!==arr(savedSections).length||expected.some((id:string)=>!actual.includes(id)))throw new Error('Tema kaydı doğrulanamadı. Mağaza verisi beklenen bölüm listesiyle eşleşmiyor.');
      designRef.current=clone(saved);sectionsRef.current=clone(arr(savedSections));setDesign(clone(saved));setSections(clone(arr(savedSections)));setThemes(nextThemes);setDirty(false);setMsg('Tasarım başarıyla kaydedildi ve mağazada yayınlandı.');
      try{setHistory(await req('/admin/design/history?storeId='+encodeURIComponent(sid)))}catch{}
      if(editorSourceMode==='demo'){try{sessionStorage.setItem('ticarti-design-save-success','Tema başarıyla mağazaya kaydedildi ve yayınlandı.')}catch{}const url=new URL(window.location.href);url.searchParams.set('sourceMode','edit');url.searchParams.set('themeId',String(targetTheme.id));url.searchParams.set('skinSlug',targetSkin);window.location.replace(url.toString());return;}
...
function LegacySectionSettings({section,update}:{section:any;update:(patch:any)=>void}){const s=section.settings||{};return <>
