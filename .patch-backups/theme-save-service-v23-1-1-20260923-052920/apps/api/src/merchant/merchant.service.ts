...
export class MerchantService {
  ...
  async publishThemeDesign(tenantId:string,storeId:string,body:any){
    const store=await this.assertStore(tenantId,storeId);
    const themeId=String(body?.themeId||'').trim();if(!themeId)throw new BadRequestException('Tema gerekli');
    const theme=await this.prisma.themeDefinition.findFirst({where:{id:themeId,isActive:true}});if(!theme)throw new NotFoundException('Tema bulunamadı');
    ...
    await this.captureThemeDesignHistory(tenantId,storeId,'MANUAL_SAVE','Kaydetmeden önce');
    ...
    const currentRoot:any=store.settings||{};const incoming:any=body?.design||{};const incomingGeneral:any={...(incoming.general||{})};delete incomingGeneral.themeName;
    const nextDesign:any={...incoming,general:{...incomingGeneral,skinSlug:requestedSkin,skin:requestedSkin,themeBundleSkinSlug:requestedSkin,themeBundleSectionsSkinSlug:requestedSkin,themeBundleVersion:'v23.0.3'}};
    const pageKey=String(body?.pageKey||'home');const items=Array.isArray(body?.sections)?body.sections:[];
    ...
    await this.prisma.$transaction(async tx=>{
      ...
      await tx.tenant.update({where:{id:storeId},data:{activeTheme:theme.slug,settings:{...currentRoot,design:nextDesign}}});
      await tx.siteSection.deleteMany({where:{storeId,pageKey}});if(normalized.length)await tx.siteSection.createMany({data:normalized});
    });
    await this.cache.delPattern('public:site:*');await this.cache.purgeTenant(tenantId);
    return {ok:true,activeTheme:theme.slug,skinSlug:requestedSkin,design:await this.designSettings(tenantId,storeId),sections:await this.designSections(tenantId,storeId,pageKey)};
  }
