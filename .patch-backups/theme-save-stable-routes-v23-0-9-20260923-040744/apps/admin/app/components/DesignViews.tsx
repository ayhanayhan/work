...
export default function DesignViewRouter({page,data,sid,req,reload}:Props){
  ...
  async function saveAll(){
    ...
    try{
      ...
      let out:any;
      try{
        out=await req('/admin/design/publish',{method:'POST',body:JSON.stringify({storeId:sid,themeId:targetTheme.id,skinSlug:targetSkin,design:nextDesign,pageKey:'home',sections:sectionPayload})});
      }catch(primary:any){
        const text=String(primary?.message||'');
        if(/Cannot POST|404|Not Found/i.test(text))throw new Error('Tema yayınlama API sürümü eski. commerce-api v23.0.8 deploy edilmeden tasarım kaydedilemez.');
        throw primary;
      }
