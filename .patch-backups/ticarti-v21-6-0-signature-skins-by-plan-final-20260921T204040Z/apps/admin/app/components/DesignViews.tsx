...
export default function DesignViewRouter({page,data,sid,req,reload}:Props){
  ...
  function previewTheme(theme:any){
    setPreviewThemeId(String(theme.id));
    sendDraft({kind:'theme',theme});
    if(theme.config?.design)sendDraft({kind:'design',design:theme.config.design});
    if(loadThemeModules&&Array.isArray(theme.config?.homePreset))sendDraft({kind:'sections',sections:theme.config.homePreset.map((x:any,i:number)=>({id:'theme-preview-'+i,pageKey:'home',sectionType:x.sectionType,sortOrder:i,enabled:x.enabled!==false,settings:x.settings||{}}))});
  }
  ...
  const themesPanel=<>
    ...
    <div className="design-v14-tree-scroll">
      ...
      <div className="design-theme-list">{themes.map((t:any)=>{const previewing=previewThemeId===String(t.id);return <div key={t.id} className={'design-theme-card '+(t.current?'active ':'')+(previewing?'previewing':'')}><div className="design-theme-preview" style={{background:`linear-gradient(135deg,${t.config?.style?.accent||'#304ffe'}22,#fff)`}}><div className="design-theme-status">{previewing&&<span className="preview-badge"><Icon icon="iconoir:eye"/> Önizleniyor</span>}{t.current&&<span className="active-badge"><Icon icon="iconoir:check-circle"/> Aktif</span>}</div><span>{t.category}</span><b>{t.name}</b><small>{Array.isArray(t.config?.homePreset)?`${t.config.homePreset.length} bölüm preset`:'Tema preset'}</small></div><div className="design-theme-meta"><div><b>{t.name}</b><small>{t.description}</small></div><em>{t.current?'Aktif':t.included?'Pakete dahil':t.owned?'Satın alındı':Number(t.price)>0?`${Number(t.price).toLocaleString('tr-TR')} ${t.currency}`:'Ücretsiz'}</em></div><div className="design-theme-actions"><button type="button" className="design-v15-light-btn" onClick={()=>previewing?closeThemePreview():previewTheme(t)}>{previewing?'Önizlemeyi Kapat':'Önizle'}</button>{!t.current&&(t.owned||t.included||Number(t.price)===0)?<button type="button" className="design-v15-primary-btn" onClick={async()=>{try{const out=await req('/admin/themes/'+t.id+'/activate',{method:'POST',body:JSON.stringify({storeId:sid,loadThemeModules})});if(out?.design)setDesign(out.design);if(Array.isArray(out?.sections))setSections(out.sections);setPreviewThemeId('');setDirty(false);setThemes(await req('/admin/themes?storeId='+encodeURIComponent(sid)));setHistory(await req('/admin/design/history?storeId='+encodeURIComponent(sid)));await reload()}catch(e:any){setThemeErr(e.message)}}}>Kullan</button>:!t.current?<button type="button" className="design-v15-primary-btn" onClick={async()=>{try{const out=await req('/admin/billing/iyzico/theme-checkout',{method:'POST',body:JSON.stringify({themeId:t.id})});if(out?.included||out?.owned){setThemes(await req('/admin/themes?storeId='+encodeURIComponent(sid)));return}if(out?.paymentUrl)window.location.href=out.paymentUrl}catch(e:any){setThemeErr(e.message)}}}>Satın al</button>:null}</div></div>})}</div>
    </div>
  </>;
