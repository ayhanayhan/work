console.log('signature commerce Theme. Version 3.4.0 commerce 2.0. https://marketplace.net/item/signature-multipurpose-commerce-theme/22559417')
...
if (typeof window.commerce == 'undefined') {
  window.commerce = {};
}

commerce.bind = function (fn, scope) {
  return function () {
    return fn.apply(scope, arguments);
  };
};

commerce.setSelectorByValue = function (selector, value) {
...
commerce.addListener = function (target, eventName, callback) {
  target.addEventListener
    ? target.addEventListener(eventName, callback, false)
    : target.attachEvent('on' + eventName, callback);
};

commerce.postLink = function (path, options) {
  options = options || {};
...
class selectOptionJs extends HTMLElement {
  constructor() {
    ...
    if(search.indexOf('variant=') == -1){
      ...
      var sect = this.closest('.commerce-section')||this.closest('.popup-modal__content__data');
...
class signatureMasonry extends HTMLElement {
  constructor() {
    super();
    ...
    commerce.designMode && setTimeout(this.resizeHandler, 100);
  }
  init() {
    this.classList.add('initialized');
    this.msnry = new Masonry(this, {
      itemSelector: '.signature-masonry__item',
      columnWidth: '.signature-masonry-grid-sizer',
...
customElements.define('masonry-items', signatureMasonry);

class signatureSwiper extends HTMLElement {
  constructor() {
    super();
    this.swiper = false;
  ...
  loadSwiper(){
    if(!document.querySelector('.swiperjs')){
      ...
      commerce.designMode&&setTimeout(this.start.bind(this), 100);
    }
  }
  isTouchDevice() {
  ...
  start(){
    ...
    if(!this.video_autoplay || commerce.designMode) return;
...
if (document.readyState === 'complete') {
  customElements.define('swiper-slider', signatureSwiper);
}
else{
...
class mainSlider extends signatureSwiper {
  constructor() {
    super();
...
class ListSpotlight extends signatureSwiper {
  constructor() {
    super();
  }
  connectedCallback() {
...
class VariantSelects extends HTMLElement {
  ...
  initMedia(){
    var _parent = this.closest('.commerce-section')||this.closest('.popup-modal__content__data');
  ...
  getParent(){
    return this.closest('.commerce-section, .popup-modal__content__data');
  }
  updateMasterId() {
...
class externalMegamenuContent extends HTMLElement {
  constructor() {
    super();
    this.init();
    commerce.designMode&&window.addEventListener('reinit_megamenu', this.init.bind(this));
  }
  init(){
    ...
    this.querySelectorAll('[data-name]').forEach((item) => {
      ...
      if(megamenu){
        ...
        commerce.designMode && document.querySelectorAll('.admin-select-menu-item').forEach((elem) => {
...
class compareButton extends mediaButtonsEvent {
  constructor() {
    super();
    this.parent = this.closest('.signature-compare');
  ...
  deleteItem(){
    ...
    setTimeout(function(){
      ...
      _this.parent.querySelector('.signature-compare__count__number').innerHTML = _this.parent.querySelectorAll('.signature-compare__container')[0].children.length;
...
(!!navigator.platform.match(/iPhone|iPod|iPad/) || commerce.designMode) && import("@theme/module-change-svg-id").then((module) => { module.changeSvgId() });
...
class mapSectionHandler extends HTMLElement {
  constructor() {
    ...
    if(!scripts.length || commerce.designMode) this.loadMapScript();
  }
  loadMapScript(){
...
class freeDeliveryBar extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.threshold = Math.round(this.threshold * (commerce.currency.rate || 1));
    ...
    if(_threshold >= this.threshold){
      ...
      pr2 = Math.round(pr2 * (commerce.currency.rate || 1));
...
class tickerHandler extends HTMLElement {
  ...
  init(){
      !commerce.designMode&&window.removeEventListener('tickerLoaded', this.initfunction);
...
class signatureSwiperReels extends HTMLElement {
  constructor() {
    super();
    this.swiper = false;
    this.classList.add('swiper-buttons-initialized');
...
customElements.define('swiper-reels', signatureSwiperReels);
...
class PromoHeader extends HTMLElement {
  ...
  connectedCallback() {
    if(this.getCookie(this.space_name) && !commerce.designMode && this.time_next > 0){
      return false;
    }
  ...
  closeHandler(e){
    ...
    if(!commerce.designMode && this.time_next > 0){
...
class quickViewHandler{
  ...
  handler(e){
    ...
    .then((responseText) => {
      ...
      if (window.commerce && commerce.PaymentButton) {
        commerce.PaymentButton.init();
      }
