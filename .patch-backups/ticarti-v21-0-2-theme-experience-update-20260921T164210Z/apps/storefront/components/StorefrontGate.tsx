...
export default function StorefrontGate({children}:{children:React.ReactNode}){
  ...
  const themeStyle:any={'--store-font-body':g.bodyFont||'Inter,ui-sans-serif,system-ui,sans-serif','--store-font-heading':g.headingFont||g.bodyFont||'Inter,ui-sans-serif,system-ui,sans-serif','--store-base-font':`${Number(g.baseFontSize||16)}px`,'--store-h1':`${Number(g.h1Size||52)}px`,'--store-h2':`${Number(g.h2Size||34)}px`,'--store-h3':`${Number(g.h3Size||24)}px`,'--store-primary':g.primaryColor||'#15171a','--store-secondary':g.secondaryColor||'#727b85','--store-text':g.textColor||'#15171a','--store-bg':g.backgroundColor||'#ffffff','--store-surface':g.surfaceColor||'#ffffff','--store-border':g.borderColor||'#e6e9ec','--store-container':`${Number(g.containerWidth||1180)}px`,'--store-section-gap':`${Number(g.sectionSpacing||64)}px`,'--store-radius':`${Number(g.borderRadius||8)}px`,'--store-button-radius':`${Number(g.buttonRadius||6)}px`,'--store-cols-desktop':String(Math.max(1,Number(p.columnsDesktop||4))),'--store-cols-tablet':String(Math.max(1,Number(p.columnsTablet||2))),'--store-cols-mobile':String(Math.max(1,Number(p.columnsMobile||2)))};
  const effectiveBoot={...boot,design};
  return <ThemeRuntime theme={boot.theme}><div className="storefront-theme" dir={String(boot.store?.locale||'').toLowerCase().startsWith('ar')?'rtl':'ltr'} lang={String(boot.store?.locale||'tr-TR')} style={themeStyle} data-product-card={p.productCardTemplate||1} data-product-page={p.productPageTemplate||1} data-category-layout={p.categoryTemplate||1} data-footer-template={f.template||1}>
    ...
    <MobileStoreNav/><CookieBanner/><MarketingPopup/>
  </div></ThemeRuntime>;
}
