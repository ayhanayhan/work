...
const themes=[
  {slug:'nova-commerce',name:'Ticarti Atlas',category:'GENEL',desc:'Tam kapsamlı vitrin, mega menü, gelişmiş ürün kartları ve alışveriş bileşenleri.',featured:true,def:true,accent:'#304ffe',radius:20,width:1600,card:'classic',header:'classic',headerTemplate:1,footerTemplate:1,preset:novaHome},
...
async function main(){
  ...
  const signaturePath=join(process.cwd(),'prisma','ticarti-signature-complete-theme-package.json');
  const signaturePackage:any=JSON.parse(readFileSync(signaturePath,'utf8'));
  for(let index=0;index<(signaturePackage.themes||[]).length;index++){
    const row:any=signaturePackage.themes[index]||{};const slug=String(row.slug||'').trim();if(!slug||!row.name)continue;
    const rawConfig:any=row.config&&typeof row.config==='object'&&!Array.isArray(row.config)?row.config:{};
    const config={...rawConfig,package:{...(rawConfig.package||{}),format:'TICARTI_THEME_PACKAGE',packageVersion:String(signaturePackage.packageVersion||'21.6.0'),importedAt:new Date().toISOString()}};
    const data:any={name:String(row.name),description:row.description?String(row.description):null,category:String(row.category||'GENEL'),previewImageUrl:row.previewImageUrl?String(row.previewImageUrl):null,version:String(row.version||signaturePackage.packageVersion||'21.6.0'),basePrice:Number(row.basePrice||0),currency:String(row.currency||'TRY'),billingType:String(row.billingType||'FREE'),isActive:row.isActive!==false,isFeatured:!!row.isFeatured,isDefault:!!row.isDefault,sortOrder:Number(row.sortOrder??100+index),config};
    if(data.isDefault)await prisma.themeDefinition.updateMany({where:{slug:{not:slug}},data:{isDefault:false}});
    await prisma.themeDefinition.upsert({where:{slug},create:{slug,...data},update:data});
  }
