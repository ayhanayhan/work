...
function withBundleMarker(design:any,skinSlug:string){const next=clone(design||{});next.general={...(next.general||{}),skinSlug,skin:skinSlug,themeBundleSkinSlug:skinSlug,themeBundleVersion:'v22.8.1'};return next}
...
function normalizeDesign(raw:any){
 ...
 if(marker!==skin.slug){
   const themeName=String(raw?.general?.themeName||source?.general?.themeName||'Ticarti');
   return {...source,general:{...(source.general||{}),themeName,skinSlug:skin.slug,skin:skin.slug,themeBundleSkinSlug:skin.slug,themeBundleVersion:'v22.8.1'}};
 }
 const merged=mergeSkinDesign(raw,skin);
 return {...merged,general:{...(merged.general||{}),skinSlug:skin.slug,skin:skin.slug,themeBundleSkinSlug:skin.slug,themeBundleVersion:'v22.8.1'}};
}
...
export default function StorefrontGate({children}:{children:React.ReactNode}){
 ...
 else if(sectionMarker===skin.slug){sections=savedSections;sectionsSource='store-bundle'}
 else {sections=sourceSections;sectionsSource='source-fallback-stale-store'}
 ...
 const contextValue={boot:effectiveBoot,design,sections,menus,isDesignPreview:preview.isPreview,sourceMode:preview.sourceMode,debug:{designSource:preview.sourceMode==='demo'?'source-demo':rawMarker===skin.slug?'store-bundle':'source-fallback-stale-design',sectionsSource,bootSectionCount:savedSections.length,bootSections:savedSections,expectedFirstSource:sourceId(sourceSections[0]),expectedFirstTitle:String(sourceSections[0]?.settings?.main_heading||sourceSections[0]?.settings?.title||'')}};
