...
export class SuperAdminService {
  ...
  async setPlanThemeSkins(id:string,body:any){
    const plan=await this.prisma.plan.findUnique({where:{id}});if(!plan)throw new NotFoundException('Paket bulunamadı');
    const requested=Array.isArray(body?.skinSlugs)?body.skinSlugs.map((x:any)=>String(x).trim()).filter(Boolean):[];
    const themes=await this.prisma.themeDefinition.findMany({where:{isActive:true},select:{config:true}});
    const allowed=new Set<string>();for(const theme of themes){const cfg:any=theme.config||{};for(const skin of Array.isArray(cfg.skins)?cfg.skins:[])if(skin?.slug)allowed.add(String(skin.slug));}
    const invalid=requested.filter((x:string)=>!allowed.has(x));if(invalid.length)throw new BadRequestException(`Bilinmeyen skin: ${invalid.slice(0,5).join(', ')}`);
    const features:any=plan.features&&typeof plan.features==='object'&&!Array.isArray(plan.features)?plan.features:{};
    return this.prisma.plan.update({where:{id},data:{features:{...features,themeSkins:Array.from(new Set(requested))}}});
  }
  ...
  async importThemes(body:any){
    const packageVersion=String(body?.packageVersion||'1.0.0');
    const incoming=Array.isArray(body?.themes)?body.themes:Array.isArray(body)?body:[];
    if(!incoming.length)throw new BadRequestException('Tema paketinde themes listesi bulunamadı');
    if(incoming.length>100)throw new BadRequestException('Tek pakette en fazla 100 tema olabilir');
    const results:any[]=[];
    for(let index=0;index<incoming.length;index++){
      const row:any=incoming[index]||{};
      const slug=String(row.slug||row.name||'').trim().toLowerCase().replace(/[^a-z0-9-]+/g,'-').replace(/^-+|-+$/g,'');
      const name=String(row.name||'').trim();
      if(!slug||!name)throw new BadRequestException(`${index+1}. tema için ad ve slug gerekli`);
      const rawConfig=row.config&&typeof row.config==='object'&&!Array.isArray(row.config)?row.config:{};
      const config={...rawConfig,package:{...(rawConfig.package||{}),format:'TICARTI_THEME_PACKAGE',packageVersion,importedAt:new Date().toISOString()}};
      const data:any={name,description:row.description?String(row.description):null,category:String(row.category||'GENEL'),previewImageUrl:row.previewImageUrl?String(row.previewImageUrl):null,version:String(row.version||packageVersion),basePrice:Number(row.basePrice||0),currency:String(row.currency||'TRY'),billingType:String(row.billingType||'ONE_TIME'),isActive:row.isActive!==false,isFeatured:!!row.isFeatured,isDefault:!!row.isDefault,sortOrder:Number(row.sortOrder??index),config};
      if(data.isDefault)await this.prisma.themeDefinition.updateMany({data:{isDefault:false}});
      const theme=await this.prisma.themeDefinition.upsert({where:{slug},create:{slug,...data},update:data});
      const tenantIds=Array.isArray(row.tenantIds)?row.tenantIds.map(String):[];
      for(const tenantId of tenantIds)await this.grantTheme(tenantId,theme.id);
      results.push({id:theme.id,slug:theme.slug,name:theme.name,modules:Array.isArray(rawConfig.modules)?rawConfig.modules.length:0,pages:rawConfig.pageTemplates&&typeof rawConfig.pageTemplates==='object'?Object.keys(rawConfig.pageTemplates).length:0});
    }
    return {imported:results.length,themes:results};
  }
  ...
  async deleteTheme(id:string,force=false){
    const current=await this.prisma.themeDefinition.findUnique({where:{id},include:{_count:{select:{installations:true}}}});if(!current)throw new NotFoundException('Tema bulunamadı');
    if(current.isDefault)throw new BadRequestException('Varsayılan tema silinemez. Önce başka bir temayı varsayılan yapın.');
    if(current._count.installations>0&&!force)throw new BadRequestException(`Tema ${current._count.installations} mağazada tanımlı. Önce mağazalardan kaldırın veya zorla silin.`);
    await this.prisma.themeDefinition.delete({where:{id}});return{deleted:true,id};
  }
  ...
  async revokeTheme(tenantId:string,themeId:string){
    const tenant=await this.prisma.tenant.findUnique({where:{id:tenantId},select:{id:true,activeTheme:true}});const theme=await this.prisma.themeDefinition.findUnique({where:{id:themeId},select:{id:true,slug:true}});if(!tenant||!theme)throw new NotFoundException('Mağaza veya tema bulunamadı');
    if(tenant.activeTheme===theme.slug)throw new BadRequestException('Yayındaki tema mağazadan kaldırılamaz. Önce başka bir tema yayınlayın.');
    await this.prisma.themeInstallation.deleteMany({where:{tenantId,themeId}});return{revoked:true};
  }
