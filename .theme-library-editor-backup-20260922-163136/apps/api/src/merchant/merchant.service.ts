...
export class MerchantService {
  ...
  async themeCatalog(tenantId:string,storeId:string){
    ...
    return themes.map((theme:any)=>{const pp=Array.isArray(theme.planPrices)?theme.planPrices[0]:null;const owned=!!theme.installations?.[0];const included=!!pp?.included;const price=included?0:Number(pp?.price??theme.basePrice??0);const sourceTheme=['nova-commerce','ticarti','signature','wokiee'].includes(String(theme.slug||'').toLowerCase());const config:any=sourceTheme?{...(theme.config||{}),engine:'ticarti-skins-v2',defaultSkin:TICARTI_DEFAULT_SKIN,activeSkin:savedSkin,skins:ticartiSkinCatalog(),design:ticartiSourceDesignForSkin(savedSkin),homePreset:ticartiSourceHomePreset(savedSkin)}:theme.config;return {id:theme.id,slug:theme.slug,name:sourceTheme?'Ticarti':theme.name,description:theme.description,category:theme.category,previewImageUrl:theme.previewImageUrl,version:theme.version,isFeatured:theme.isFeatured,isDefault:theme.isDefault,current:store.activeTheme===theme.slug,owned:owned||included,included,price,currency:pp?.currency||theme.currency,billingType:theme.billingType,config};});
  }
  ...
  async activateTheme(tenantId:string,storeId:string,themeId:string,options:any={}){
    ...
    const preset:any[]=isTicartiTheme?ticartiSourceHomePreset(requestedSkin):(Array.isArray(cfg.homePreset)?cfg.homePreset:[]);
    const themeDesign:any=isTicartiTheme?ticartiSourceDesignForSkin(requestedSkin):(cfg.design||{});
