...
export default function Checkout(){
  ...
  async function loadAdmin1(codeValue:string){setAdmin1('');setAdmin2('');setAdmin2Rows([]);try{setAdmin1Rows(await api(`/storefront/${data?.store?.publicSlug||'main'}/locations?countryCode=${encodeURIComponent(codeValue)}&level=ADMIN1`))}catch{setAdmin1Rows([])}}
  async function changeCountry(codeValue:string){setCountry(codeValue);await loadAdmin1(codeValue)}
  async function changeAdmin1(id:string){setAdmin1(id);setAdmin2('');try{setAdmin2Rows(id?await api(`/storefront/${data?.store?.publicSlug||'main'}/locations?parentId=${encodeURIComponent(id)}&level=ADMIN2`):[])}catch{setAdmin2Rows([])}}
  ...
  async function loadShippingAdmin1(codeValue:string){setShippingAdmin1('');setShippingAdmin2('');setShippingAdmin2Rows([]);try{setShippingAdmin1Rows(await api(`/storefront/${data?.store?.publicSlug||'main'}/locations?countryCode=${encodeURIComponent(codeValue)}&level=ADMIN1`))}catch{setShippingAdmin1Rows([])}}
  async function changeShippingCountry(codeValue:string){setShippingCountry(codeValue);await loadShippingAdmin1(codeValue)}
  async function changeShippingAdmin1(id:string){setShippingAdmin1(id);setShippingAdmin2('');try{setShippingAdmin2Rows(id?await api(`/storefront/${data?.store?.publicSlug||'main'}/locations?parentId=${encodeURIComponent(id)}&level=ADMIN2`):[])}catch{setShippingAdmin2Rows([])}}
  ...
  async function selectSavedAddress(address:any){
    ...
    await hydrateGeo(address,data?.store?.defaultCountry||'TR',data?.store?.publicSlug||'main');
  }
  async function newAddress(){
    setSelectedAddressId('');setSelectedAddress(null);setEditingAddress(true);setPhone(customer?.phone||'');setAddressFormKey(v=>v+1);
    await hydrateGeo(null,data?.store?.defaultCountry||'TR',data?.store?.publicSlug||'main');
  }
  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    ...
    if(mode==='REQUIRED'&&!customerToken){router.push('/account?mode=login&return=/checkout');return}
  ...
  if(done){
    ...
    return <div className="checkout-shell checkout-stepflow checkout-success-page">
      <main className="checkout-success-layout">
        <header className="checkout-success-header"><Link href="/" className="checkout-brand">{data?.store?.logoUrl?<img src={data.store.logoUrl} alt={data.store.name}/>:<span>{data?.store?.name||'Mağaza'}</span>}</Link></header>
        <section className="checkout-success-main">
          <div className="checkout-success-primary">
            ...
            {registerDone&&<div className="success-register-card success-register-done"><h2>Hesabınız oluşturuldu</h2><p>Siparişinizi ve adreslerinizi hesabınızdan yönetebilirsiniz.</p><Link className="btn" href="/account">Hesabıma git</Link></div>}
  ...
  if(!cart?.items?.length)return <div className="checkout-shell"><main className="checkout-success"><div className="checkout-success-card"><h1>Sepetiniz boş</h1><Link className="btn" href="/products">Ürünlere dön</Link></div></main></div>;
      ...
      <form ref={formRef} className="checkout-final-grid" onSubmit={submit}>
        <div className="checkout-form-column">
          <div className="checkout-left-top">
            <Link href="/" className="checkout-brand">{data.store?.logoUrl?<img src={data.store.logoUrl} alt={data.store.name}/>:<span>{data.store?.name||'Mağaza'}</span>}</Link>
            {!customer?<span className="checkout-login-link">Zaten hesabınız var mı? <Link href="/account?mode=login&return=/checkout">Giriş yap</Link></span>:<span className="checkout-signed">{customer.email}</span>}
          </div>
          ...
          {mode==='REQUIRED'&&!customerToken&&<div className="checkout-auth-required"><b>Bu mağazada ödeme öncesi giriş zorunlu.</b><span>Checkout içinde üyelik oluşturulmaz. Giriş yaptıktan sonra bu sayfaya geri dönersiniz.</span><Link className="btn" href="/account?mode=login&return=/checkout">Giriş yap</Link></div>}
            ...
            <div className="checkout-step-body">
              ...
              <div className="checkout-payment-extras">
                ...
                <div className="checkout-consents">
                  ...
                  <label><input type="checkbox" name="terms" required/> <span>{contracts.DISTANCE_SALES?.href?<><Link href={contracts.DISTANCE_SALES.href} target="_blank">Mesafeli Satış Sözleşmesi</Link>ni</>:<>Mesafeli Satış Sözleşmesini</>}, {contracts.PRIVACY?.href?<><Link href={contracts.PRIVACY.href} target="_blank">Gizlilik Politikası</Link>nı</>:<>Gizlilik Politikasını</>} ve {contracts.KVKK_NOTICE?.href?<><Link href={contracts.KVKK_NOTICE.href} target="_blank">KVKK Aydınlatma Metni</Link>ni</>:<>KVKK Aydınlatma Metnini</>} okudum, onaylıyorum.</span></label>
                </div>
          ...
          <div className="checkout-left-footer"><div className="checkout-footer-links">{contracts.RETURNS?.href?<Link href={contracts.RETURNS.href}>İade Politikası</Link>:<span>İade Politikası</span>}<span>•</span>{contracts.PRIVACY?.href?<Link href={contracts.PRIVACY.href}>Gizlilik Politikası</Link>:<span>Gizlilik Politikası</span>}<span>•</span>{contracts.TERMS?.href?<Link href={contracts.TERMS.href}>Hizmet Şartları</Link>:<span>Hizmet Şartları</span>}</div><div className="checkout-footer-meta"><img className="banksicon" src="/banksicon.png" alt="Desteklenen ödeme yöntemleri"/><a className="designed-by" href="https://wediditagency.com" target="_blank" rel="noreferrer">designed by wediditagency</a></div></div>
        </div>
