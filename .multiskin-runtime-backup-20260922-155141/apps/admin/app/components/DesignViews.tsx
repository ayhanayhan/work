...
export default function DesignViewRouter({page,data,sid,req,reload}:Props){
  ...
  const[history,setHistory]=useState<any[]>([]);const[historyErr,setHistoryErr]=useState('');const[loadThemeModules,setLoadThemeModules]=useState(true);
  ...
  const themesPanel=<>
    <SidebarSubHeader title="Tema Kataloğu" subtitle="Tema görünümünü ve modül düzenini seçin" onBack={panelBack}/>
    <div className="design-v14-tree-scroll">
      {themeErr&&<div className="design-v15-inline-error">{themeErr}</div>}
      <label className="design-theme-import-option"><input type="checkbox" checked={loadThemeModules} onChange={e=>setLoadThemeModules(e.target.checked)}/><span><b>Tema modüllerini ve örnek içeriği yükle</b><small>Açıkken anasayfa tema düzeniyle yeniden kurulur. Kapalıyken mevcut modülleriniz korunur ve eşleşen modüllere tema görünümü uygulanır.</small></span></label>
      <div className="design-theme-list">{themes.map((t:any)=><div key={t.id} className={'design-theme-card '+(t.current?'active':'')}><div className="design-theme-preview" style={{background:`linear-gradient(135deg,${t.config?.style?.accent||'#304ffe'}22,#fff)`}}><span>{t.category}</span><b>{t.name}</b><small>{Array.isArray(t.config?.homePreset)?`${t.config.homePreset.length} bölüm preset`:'Tema preset'}</small></div><div className="design-theme-meta"><div><b>{t.name}</b><small>{t.description}</small></div><em>{t.current?'Aktif':t.included?'Pakete dahil':t.owned?'Satın alındı':Number(t.price)>0?`${Number(t.price).toLocaleString('tr-TR')} ${t.currency}`:'Ücretsiz'}</em></div><div className="design-theme-actions"><button type="button" className="design-v15-light-btn" onClick={()=>{const frame=previewRef.current?.contentWindow;if(!frame)return;frame.postMessage({source:'commerce-design-editor',action:'draft',kind:'theme',theme:t},'*');if(t.config?.design)frame.postMessage({source:'commerce-design-editor',action:'draft',kind:'design',design:t.config.design},'*');if(loadThemeModules&&Array.isArray(t.config?.homePreset))frame.postMessage({source:'commerce-design-editor',action:'draft',kind:'sections',sections:t.config.homePreset.map((x:any,i:number)=>({id:'theme-preview-'+i,pageKey:'home',sectionType:x.sectionType,sortOrder:i,enabled:x.enabled!==false,settings:x.settings||{}}))},'*')}}>Önizle</button>{t.current?<button type="button" className="design-v15-primary-btn" onClick={async()=>{try{const out=await req('/admin/themes/'+t.id+'/activate',{method:'POST',body:JSON.stringify({storeId:sid,loadThemeModules:true})});if(out?.design)setDesign(out.design);if(Array.isArray(out?.sections))setSections(out.sections);setDirty(false);setThemes(await req('/admin/themes?storeId='+encodeURIComponent(sid)));setHistory(await req('/admin/design/history?storeId='+encodeURIComponent(sid)));await reload()}catch(e:any){setThemeErr(e.message)}}}>Tema içeriğini yeniden yükle</button>:!t.current&&(t.owned||t.included||Number(t.price)===0)?<button type="button" className="design-v15-primary-btn" onClick={async()=>{try{const out=await req('/admin/themes/'+t.id+'/activate',{method:'POST',body:JSON.stringify({storeId:sid,loadThemeModules})});if(out?.design)setDesign(out.design);if(Array.isArray(out?.sections))setSections(out.sections);setDirty(false);setThemes(await req('/admin/themes?storeId='+encodeURIComponent(sid)));setHistory(await req('/admin/design/history?storeId='+encodeURIComponent(sid)));await reload()}catch(e:any){setThemeErr(e.message)}}}>Kullan</button>:!t.current?<button type="button" className="design-v15-primary-btn" onClick={async()=>{try{const out=await req('/admin/billing/iyzico/theme-checkout',{method:'POST',body:JSON.stringify({themeId:t.id})});if(out?.included||out?.owned){setThemes(await req('/admin/themes?storeId='+encodeURIComponent(sid)));return}if(out?.paymentUrl)window.location.href=out.paymentUrl}catch(e:any){setThemeErr(e.message)}}}>Satın al</button>:null}</div></div>)}</div>
    </div>
  </>;
