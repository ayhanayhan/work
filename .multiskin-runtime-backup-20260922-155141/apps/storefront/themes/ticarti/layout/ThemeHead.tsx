...
export default function ThemeHead({design,children}:{design:any;children:React.ReactNode}){
 ...
 const vars=useMemo(()=>({...ticartiCssVariables(settings),...lingerieCssVariables,...(explicit?ticartiCssVariables(settings):{})}) as any,[settings,explicit]);
 useEffect(()=>{const families=explicit?googleFontFamilies(settings):['Roboto Flex','Prata'];const id='ticarti-theme-google-fonts';const old=document.getElementById(id) as HTMLLinkElement|null;if(!families.length){old?.remove();return}const href='https://fonts.googleapis.com/css2?'+families.map(x=>`family=${encodeURIComponent(x).replace(/%20/g,'+')}:wght@300;400;500;600;700;800;900`).join('&')+'&display=swap';if(old?.href===href)return;const el=old||document.createElement('link');el.id=id;el.rel='stylesheet';el.href=href;if(!old)document.head.appendChild(el)},[settings]);
 return <div className="ticarti-theme" data-theme-id="ticarti" data-theme-name={String((settings as any).themeName||'Ticarti')} data-theme-skin="18-lingerie-store" style={vars}>
   ...
   <link rel="stylesheet" href="/themes/ticarti/lingerie-generated.css"/>
   {children}
 </div>;
}
