...
export class MerchantService {
  ...
  async themeCatalog(tenantId:string,storeId:string){
    const store=await this.assertStore(tenantId,storeId);
    const sub=await this.prisma.subscription.findFirst({where:{tenantId,status:{in:['ACTIVE','TRIAL']}},orderBy:{createdAt:'desc'},select:{planId:true,plan:{select:{features:true}}}});
    const features:any=sub?.plan?.features||{};
    const configuredSkinAccess:any[]=Array.isArray(features.themeSkins)?features.themeSkins:[];
    // Backwards-compatible default: until Super Admin explicitly assigns skins to a plan,
    // merchants can see the complete skin catalog. Once one or more slugs are assigned,
    // the catalog is restricted to that exact plan list (or '*' for all).
    const rawSkinAccess:any[]=configuredSkinAccess.length?configuredSkinAccess:['*'];
    const skinAccess=new Set(rawSkinAccess.map((x:any)=>String(x)));
    const allSkins=skinAccess.has('*');
    const root:any=store.settings||{};
    const activeSkin=String(root.themeSkin?.slug||root.design?.general?.skinSlug||'');
    ...
    return themes.filter((theme:any)=>{const cfg:any=theme.config||{};return cfg.visibility!=='PRIVATE'||!!theme.installations?.[0]||store.activeTheme===theme.slug}).map((theme:any)=>{
      const pp=Array.isArray(theme.planPrices)?theme.planPrices[0]:null;const owned=!!theme.installations?.[0];const included=!!pp?.included;const price=included?0:Number(pp?.price??theme.basePrice??0);const cfg:any=theme.config||{};
      const catalogSkins:any[]=Array.isArray(cfg.skins)?cfg.skins:[];
      const visibleSkins=catalogSkins.filter((skin:any)=>allSkins||skinAccess.has(String(skin?.slug||''))||(store.activeTheme===theme.slug&&String(skin?.slug||'')===activeSkin));
      if(catalogSkins.length&&!visibleSkins.length&&store.activeTheme!==theme.slug)return null;
      return {id:theme.id,slug:theme.slug,name:theme.name,description:theme.description,category:theme.category,previewImageUrl:theme.previewImageUrl,version:theme.version,isFeatured:theme.isFeatured,isDefault:theme.isDefault,current:store.activeTheme===theme.slug,activeSkin:store.activeTheme===theme.slug?activeSkin:'',owned:owned||included,included,price,currency:pp?.currency||theme.currency,billingType:theme.billingType,config:{...cfg,skins:visibleSkins}};
    }).filter(Boolean);
  }
  private async captureThemeDesignHistory(tenantId:string,storeId:string,reason='MANUAL_SAVE',label?:string){
    ...
    const snapshot={activeTheme:store.activeTheme,themeSkin:settings.themeSkin||null,design:settings.design||await this.designSettings(tenantId,storeId),sections:sections.map((x:any)=>({pageKey:x.pageKey,sectionType:x.sectionType,sortOrder:x.sortOrder,enabled:x.enabled,settings:x.settings}))};
  ...
  async activateTheme(tenantId:string,storeId:string,themeId:string,options:any={}){
    ...
    const cfg:any=theme.config||{};const visibleSkins:any[]=Array.isArray(item?.config?.skins)?item.config.skins:[];const root:any=store.settings||{};const requestedSkin=String(options?.skinSlug||'').trim();const currentSkin=String(root.themeSkin?.slug||root.design?.general?.skinSlug||'');
    let selectedSkin:any=null;
    if(Array.isArray(cfg.skins)&&cfg.skins.length){
      if(requestedSkin)selectedSkin=visibleSkins.find((skin:any)=>String(skin?.slug||'')===requestedSkin);
      else selectedSkin=visibleSkins.find((skin:any)=>String(skin?.slug||'')===currentSkin)||visibleSkins.find((skin:any)=>String(skin?.slug||'')===String(cfg.defaultSkin||''))||visibleSkins[0];
      if(!selectedSkin)throw new BadRequestException('Bu skin mevcut paketinize dahil değil.');
    }
    await this.captureThemeDesignHistory(tenantId,storeId,'THEME_CHANGE',`${store.activeTheme} → ${theme.slug}${selectedSkin?` / ${selectedSkin.name}`:''}`);
    const preset:any[]=Array.isArray(selectedSkin?.homePreset)?selectedSkin.homePreset:(Array.isArray(cfg.homePreset)?cfg.homePreset:[]);const themeDesign:any=cfg.design||{};const skinDesign:any=selectedSkin?.design||{};const defaults:any=this.defaultDesignSettings();const nextDesign={general:{...defaults.general,...(themeDesign.general||{}),...(skinDesign.general||{}),...(selectedSkin?{skinSlug:selectedSkin.slug,skinName:selectedSkin.name,skinGroup:selectedSkin.group}: {})},header:{...defaults.header,...(themeDesign.header||{}),...(skinDesign.header||{})},footer:{...defaults.footer,...(themeDesign.footer||{}),...(skinDesign.footer||{})},products:{...defaults.products,...(themeDesign.products||{}),...(skinDesign.products||{})}};
    // Skin seçildiğinde o skinin modül sırası da varsayılan olarak yüklenir. Yalnızca API'de açıkça false gönderilirse mevcut bölümler korunur.
    ...
    await this.prisma.$transaction(async tx=>{
      ...
      await tx.tenant.update({where:{id:store.id},data:{activeTheme:theme.slug,settings:{...root,themeSkin:selectedSkin?{slug:selectedSkin.slug,name:selectedSkin.name,group:selectedSkin.group}:null,design:nextDesign}}});
      if(loadThemeModules){
    ...
    await this.cache.delPattern('public:site:*');await this.cache.purgeTenant(tenantId);return {activeTheme:theme.slug,activeSkin:selectedSkin?.slug||null,loadThemeModules,design:nextDesign,sections:await this.designSections(tenantId,storeId,'home')};
  }
