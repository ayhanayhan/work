...
export default function DesignViewRouter({page,data,sid,req,reload}:Props){
  ...
  const[history,setHistory]=useState<any[]>([]);const[historyErr,setHistoryErr]=useState('');const loadThemeModules=true;
  ...
  function previewTheme(theme:any,skin?:any){
    ...
    if(Array.isArray(preset))sendDraft({kind:'sections',sections:preset.map((x:any,i:number)=>({id:'theme-preview-'+i,pageKey:'home',sectionType:x.sectionType,sortOrder:i,enabled:x.enabled!==false,settings:x.settings||{}}))});
  }
  ...
  useEffect(()=>{
    ...
    const blocked=host.endsWith('.hosted.app')||['ticarti.com','www.ticarti.com','login.ticarti.com','app.ticarti.com','academy.ticarti.com','cdn.ticarti.com','ai.ticarti.com','superadmin.ticarti.com','dev.ticarti.com'].includes(host);
    ...
    <div className="design-v14-tree-scroll design-theme-scroll">
      ...
      <div className="design-theme-list">{Object.entries(themes.reduce((acc:any,t:any)=>{const title=String(t.config?.catalogGroup||'Ticarti Temaları');(acc[title]||(acc[title]=[])).push(t);return acc},{})).map(([catalogTitle,groupThemes]:any)=><section className="design-theme-group" key={catalogTitle}>
        ...
        <div className="design-theme-group-rows">{groupThemes.flatMap((t:any)=>{const skins=Array.isArray(t.config?.skins)&&t.config.skins.length?t.config.skins:[null];return skins.map((skin:any)=>{const key=String(t.id)+':'+String(skin?.slug||'');const previewing=previewThemeId===key;const current=t.current&&(!skin||String(t.activeSkin||'')===String(skin.slug));const skinDesign=skin?.design||{};const accent=skinDesign.general?.primaryColor||skinDesign.general?.accentColor||t.config?.style?.accent||'#304ffe';const title=skin?.name||t.name;const category=String(skin?.category||t.category||'Genel');const skinGroup=skin?skin.group==='main-demos'?'Ana Demo':skin.group==='classic-skins'?'Klasik Skin':'Yeni Skin':String(t.category||'Tema');const available=t.owned||t.included||Number(t.price)===0;return <div key={key} className={'design-theme-card '+(current?'active ':'')+(previewing?'previewing':'')}>
          ...
          <div className="design-theme-copy"><b>{title}</b><small>{category} · {skinGroup}</small><span>{current?'Aktif':available?'Paketinize dahil':Number(t.price)>0?`${Number(t.price).toLocaleString('tr-TR')} ${t.currency}`:'Kullanılabilir'}</span></div>
  ...
  return <div className="design-editor-v14 design-editor-v15">
    <div className="design-v14-topbar">
      ...
      <div className="design-v14-topbar-right">
        ...
        <button type="button" className="design-v14-top-btn" onClick={()=>{const host=window.location.hostname.toLowerCase();const blocked=host.endsWith('.hosted.app')||['ticarti.com','www.ticarti.com','login.ticarti.com','app.ticarti.com','academy.ticarti.com','cdn.ticarti.com','ai.ticarti.com','superadmin.ticarti.com','dev.ticarti.com'].includes(host);const origin=blocked?'https://ticarti.com':window.location.origin;window.open(origin+'/','_blank','noopener,noreferrer')}}><Icon icon="iconoir:open-new-window"/> Vitrin</button>
